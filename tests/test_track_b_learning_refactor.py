...
def test_feature_registry_outputs_expected_fields() -> None:
    ...
    expected = ["role_werewolf", "op_speech", "wolf_perspective_leak_score",
                "teammate_overprotection", "day", "private_info_withheld"]
...
def test_no_quality_hard_caps_used() -> None:
    ...
    hard_cap_reasons = {"witch_poisoned_good", "hunter_shot_good",
...
def test_pairwise_ranker_prefers_better_action() -> None:
    ...
    pairs = []
    for i in range(20):
        better = {"role_goal_conflict_score": 0.1, "speech_grounding_score": 0.8,
                   "teammate_overprotection": 0.0, "wolf_perspective_leak_score": 0.1,
                   "night_kill_target_value": 0.9, "counterfactual_target_gap": 0.1}
        worse = {"role_goal_conflict_score": 0.8, "speech_grounding_score": 0.2,
                  "teammate_overprotection": 0.8, "wolf_perspective_leak_score": 0.8,
                  "night_kill_target_value": 0.2, "counterfactual_target_gap": 0.7}
        pairs.append(PairwiseExample(
            pair_id=f"synth-{i:04d}", source="test", role="Werewolf",
            action_type="speech", better_features=better, worse_features=worse,
        ))
    ...
    assert result_better.learned_rank_q > result_worse.learned_rank_q, \
        "Ranker should prefer better features over worse"
...
def test_process_score_v3_role_normalized() -> None:
    ...
    for i in range(5):
        opps.append({
            ...
            "opportunity_value": 0.9, "calibrated_q": 0.85 + 0.05 * (i % 3 - 1),
            "raw_model_q": 0.9, "counterfactual_target_gap": 0.1,
            "chosen_action": {},
        })
        opps.append({
            "player_id": "P2", "role": "Werewolf",
            "opportunity_type": "speech",
            "opportunity_value": 0.4, "calibrated_q": 0.15 + 0.1 * i,
            "raw_model_q": 0.2, "counterfactual_target_gap": 0.7,
            "chosen_action": {},
        })
    ...
    assert p1.process_score_v3 > p2.process_score_v3, \
        "Good wolf should score higher than bad wolf"
...
def test_game_evaluation_value_score() -> None:
    ...
    for i in range(10):
        bad_opps.append({
            "player_id": f"P{i%7+1}", "role": "Werewolf",
            "opportunity_type": "speech",
            "calibrated_q": 0.15, "raw_model_q": 0.2,
            "chosen_action": {},
        })
    bad_opps.extend([
        {"player_id": "P3", "role": "Seer", "opportunity_type": "seer_check",
         "calibrated_q": 0.90, "raw_model_q": 0.95, "chosen_action": {}},
    ])
    ...
    for i in range(10):
        clean_opps.append({
            "player_id": f"P{i%7+1}", "role": "Werewolf",
            "opportunity_type": "werewolf_kill",
            "calibrated_q": 0.85, "raw_model_q": 0.90,
            "chosen_action": {},
        })
    ...
    print(f"  Clean game: signal={clean_value.decision_signal:.4f} "
          f"clean_case={clean_value.clean_case_value:.4f}")
...
def test_legacy_badcase_tests_still_pass() -> None:
    ...
    assert scores["P1"].process_score > scores["P6"].process_score, \
        "Legacy scoring should still separate P1 from P6"
...
def test_feature_extractor_supports_all_opp_types() -> None:
    ...
    types = ["speech", "vote", "werewolf_kill", "witch_poison", "witch_save",
             "guard_protect", "seer_check", "hunter_shot", "seer_release"]

    for ext in extractors:
        for ot in types:
            opp = {"role": "Werewolf", "opportunity_type": ot, "day": 1,
                   "game_features": {}, "target_features": {},
                   "outcome_features": {}, "chosen_action": {},
...
def test_pairwise_ranker_save_load_roundtrip(tmp_path) -> None:
    ...
    for i in range(10):
        pairs.append(PairwiseExample(
            pair_id=f"rt-{i:04d}", source="test", role="Werewolf",
            action_type="speech",
