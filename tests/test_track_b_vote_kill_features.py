...
def test_kill_target_features_produce_delta():
    ...
    bb = ReplayBundleBuilder().build(bad_state); cb = ReplayBundleBuilder().build(cc_state)
    b_opps = [op.to_dict() for op in OpportunityExtractor().extract(bb)
              if op.opportunity_type == "werewolf_kill" and op.day == 2]
    c_opps = [op.to_dict() for op in OpportunityExtractor().extract(cb)
              if op.opportunity_type == "werewolf_kill" and op.day == 2]

    assert b_opps and c_opps
    bf = reg.extract(b_opps[0]).features; gf = reg.extract(c_opps[0]).features
    all_keys = set(list(bf.keys()) + list(gf.keys()))
    deltas = 0
    for k in all_keys:
        bv = bf.get(k, 0); gv = gf.get(k, 0)
