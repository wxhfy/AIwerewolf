...
| 知识回流 | 从复盘中抽取 lesson，进入 candidate / active / deprecated 生命周期 | `backend/eval/knowledge_abstractor.py`, `backend/eval/evolution.py` |
...
| 把历史经验直接塞进 Prompt | 噪声高、成本高，也可能污染当前局信息边界 | StrategyRetriever 经过 confidence / visibility / privacy / applicability 过滤后按需注入 |
...
Track C 将复盘结果转成策略知识。新知识默认进入 `candidate`，不会直接污染 `active` 策略池。后续可以通过质量筛选、聚类、使用反馈或实验对比晋级为 `active`，也可以因效果差或冲突被降级为 `deprecated`。
...
Track C 的策略知识和回流能力通过后端 API、脚本和报告材料呈现，不承诺独立前端页面。
...
6. 知识回流：candidate -> active -> StrategyRetriever -> 下一局策略注入。
