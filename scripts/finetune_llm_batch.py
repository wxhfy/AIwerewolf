...
def batch_verify(pairs, batch_size=8):
    ...
    prompt = "判断以下每对策略是否属于【不同主题】。如果讨论的是同一类问题→false_negative，完全不同主题→true_negative。\n\n" + "\n\n".join(items) + "\n\n返回JSON数组: [{\"id\":1,\"is_true_negative\":true/false,\"confidence\":0.0-1.0},...] 只返回JSON。"

    try:
        resp = requests.post(LLM_URL,
            headers={"Authorization": f"Bearer {LLM_KEY}", "Content-Type": "application/json"},
            json={"model": LLM_MODEL, "messages": [{"role": "user", "content": prompt}], "temperature": 0.1, "max_tokens": 500},
            timeout=60)
...
def generate_verified_triplets(docs, n_samples=300):
    ...
    role_idx = {}; [role_idx.setdefault(d["role"],[]).append(i) for i,d in enumerate(docs)]

    # First, generate all candidates
    candidates = []
    for i, anchor in enumerate(docs):
        if len(candidates) >= n_samples * 2: break
    ...
    for bi in range(0, min(len(candidates), n_samples*2), 8):
        ...
        if (bi//8+1) % 5 == 0:
            print(f"    Verified {bi+len(batch)}/{min(len(candidates), n_samples*2)}, {len(verified_triplets)} accepted")
    ...
    with open("data/ft_llm_verified.jsonl", "w") as f:
        for t in verified_triplets: f.write(json.dumps(t, ensure_ascii=False)+"\n")
    return verified_triplets

class TripletDataset(Dataset):
    def __init__(self, triplets, tokenizer, max_len=256):
        self.triplets = triplets; self.tokenizer = tokenizer; self.max_len = max_len
    ...
    def collate(self, batch):
        ...
        return self.tokenizer(texts, padding=True, truncation=True, max_length=self.max_len, return_tensors="pt"), len(batch)
...
def finetune(triplets, output_path=OUTPUT_PATH, epochs=2, batch_size=12, lr=2e-5):
    ...
    model = SentenceTransformer(BGE_PATH, device=GPU); model.train()
    ...
    model.to(device); best_loss = float('inf')
    for ep in range(epochs):
        ...
        for step, (enc, n) in enumerate(dl):
            ...
            ep_loss += loss.item()
        avg = ep_loss/len(dl); print(f"  Epoch {ep+1}: Loss={avg:.4f}")
        if avg < best_loss:
            best_loss = avg; os.makedirs(output_path, exist_ok=True)
            import shutil
...
def evaluate_model(model_path, docs, queries, label=""):
    ...
    model = SentenceTransformer(model_path, device=GPU); N = len(docs)
    ...
    for q in queries:
        ...
        for k in [5,10]:
            kk=min(k,len(res))
            m[f"P@{k}"] = sum(1 for r in res[:kk] if sum(1 for kw in q["kw"] if kw.lower() in f"{r['situation']} {r['strategy']}".lower())>0)/kk
        for rnk,r in enumerate(res,1):
            if sum(1 for kw in q["kw"] if kw.lower() in f"{r['situation']} {r['strategy']}".lower())>0: m["MRR"]=1.0/rnk; break
        else: m["MRR"]=0.0
        for k in [5,10]:
            kk=min(k,len(res))
            gains = [sum(1 for kw in q["kw"] if kw.lower() in f"{r['situation']} {r['strategy']}".lower()) for r in res[:kk]]
            dcg = sum(g/np.log2(j+2) for j,g in enumerate(gains))
            gs = sorted(gains, reverse=True)
            m[f"NDCG@{k}"] = dcg/sum(g/np.log2(j+2) for j,g in enumerate(gs)) if sum(g/np.log2(j+2) for j,g in enumerate(gs))>0 else 0.0
        metrics.append(m)
