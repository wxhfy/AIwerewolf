...
当前不为每一步拉取子 Agent，也不在实时决策中执行赛后分析。默认 `direct` 模式下，普通行动使用一步主模型调用，高影响行动最多增加一步受预算约束的反思。实验性 `agentic` 模式允许在提交动作前加载一个可信 Skill、调用一个只读角色视角工具；它必须经过配对实验确认质量收益高于额外时延后，才适合扩大启用范围。

记忆更新、基础发言证据提取、合法性校验和状态推进仍使用确定性代码。复杂语义解析和 Track C 多 Agent 搜索属于异步增强链路，不进入规则事务。
...
WerewolfGame 到达决策点
  ...
  -> AgentHarness.run(...)
  -> LLMActionPlanner 直决策，或受预算约束的反思 / Skill / 只读工具微循环
...
Planner 会携带 `context_manifest`，记录各层包含数量、裁剪上限、剩余步数和 Deadline。详细的层级、优先级、开源 Harness 参考与压缩方向见 [`AGENT_HARNESS_V2.md`](AGENT_HARNESS_V2.md)。

### 5.4 为什么实时对局不使用子 Agent
...
### 5.5 Track C 为什么适合子 Agent

Track C 是赛后离线流程，不受单步发言 Deadline 约束，适合把证据抽取、候选策略生成、红队审查和多 Seed Tournament 拆成并行任务。Coordinator 负责 DAG、幂等和唯一写入；子 Agent 使用隔离上下文并只返回结构化 Artifact，不直接修改活动策略版本。这样能扩大候选搜索并横向扩展实验吞吐，同时不污染实时对局。
...
当前 Pipeline 已具备纯 AI 对局的主要闭环。2026-09-21 已使用免费 `THUDM/GLM-4-9B-0414` 完成 7 人真实整局：46 次模型决策、0 次修复、0 次兜底，正常进入 `GAME_END`。验证细节见 [`MODEL_VALIDATION_GLM.md`](MODEL_VALIDATION_GLM.md)。
...
8. 建立发言语义标注集，补齐复杂声明、条件、指代和讽刺理解。
9. 用人工标注校准 Track B Judge，并实现 Track C 并行 Coordinator。
