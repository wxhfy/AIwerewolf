...
def finetune_simple(triplets, model_path=BGE_PATH, output_path=OUTPUT_PATH, epochs=3, batch_size=16, lr=2e-5):
    ...
    print(f"Training: {len(triplets)} triplets, {epochs} epochs, batch={batch_size}, lr={lr}, steps={total_steps}")
...
def evaluate_model(model_path, docs, queries, label=""):
    ...
    print(f"\n  {label}:")
    print(
        f"    NDCG@5={agg['NDCG@5']:.3f} P@5={agg['P@5']:.3f} MRR={agg['MRR']:.3f} "
