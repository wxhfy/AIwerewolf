...
TEST_QUERIES = [
    {"id": "Q1", "role": "Seer", "phase": "DAY_BADGE_SPEECH", "query_text": "我是预言家现在竞选警长需要报警徽流和查验结果", "keywords": ["警徽流", "警长竞选", "预言家警上", "报查验"]},
    {"id": "Q2", "role": "Witch", "phase": "NIGHT_WITCH_ACTION", "query_text": "第一晚有人被刀我要决定是否使用解药救人", "keywords": ["首夜解药", "第一晚救人", "解药使用", "盲救"]},
    {"id": "Q3", "role": "Werewolf", "phase": "NIGHT_WOLF_ACTION", "query_text": "今晚我们狼队要选择击杀目标应该优先刀谁", "keywords": ["刀人优先级", "击杀目标", "刀法", "屠边"]},
    {"id": "Q4", "role": "Werewolf", "phase": "DAY_SPEECH", "query_text": "我是狼人需要伪装成好人发言不能被发现", "keywords": ["伪装好人", "狼人伪装", "模仿好人", "隐藏狼人身份"]},
    {"id": "Q5", "role": "Villager", "phase": "DAY_SPEECH", "query_text": "我是平民需要给出有价值的发言来分析局势", "keywords": ["平民发言", "发言质量", "村民发言", "分析发言"]},
    {"id": "Q6", "role": "Hunter", "phase": "DAY_SPEECH", "query_text": "我被多人投票可能要出局了要不要跳猎人身份", "keywords": ["猎人跳身份", "开枪威慑", "枪徽流", "被票"]},
    {"id": "Q7", "role": "Seer", "phase": "DAY_SPEECH", "query_text": "有人对跳预言家我需要证明自己是真的", "keywords": ["对跳预言家", "真假预言家", "悍跳", "对跳", "查验链"]},
    {"id": "Q8", "role": "Werewolf", "phase": "DAY_SPEECH", "query_text": "我被真预言家查杀了应该怎么应对", "keywords": ["被查杀", "反跳预言家", "被真预言家查杀", "自爆"]},
    {"id": "Q9", "role": "Witch", "phase": "mid_game", "query_text": "已经第三天了我有一瓶毒药还没用应该毒谁", "keywords": ["毒药使用", "开毒", "撒毒", "毒人时机"]},
    {"id": "Q10", "role": "Guard", "phase": "NIGHT_GUARD_ACTION", "query_text": "今晚我要守护一个人不能连续守同一个", "keywords": ["守护目标", "轮换守护", "守卫守护", "不能连续"]},
    {"id": "Q11", "role": "WhiteWolfKing", "phase": "DAY_SPEECH", "query_text": "我是白狼王什么时候自爆带走关键好人最合适", "keywords": ["白狼王自爆", "自爆时机", "自爆带人", "吞警徽"]},
    {"id": "Q12", "role": "global", "phase": "DAY_VOTE", "query_text": "投票环节怎样分析票型来找到狼人", "keywords": ["票型分析", "投票记录", "变票者", "冲票"]},
    {"id": "Q13", "role": "global", "phase": "late_game", "query_text": "残局阶段只剩3个人了怎么做出最后正确的投票抉择", "keywords": ["残局策略", "残局抉择", "最后抉择", "残局"]},
    {"id": "Q14", "role": "Werewolf", "phase": "DAY_SPEECH", "query_text": "我的狼队友被怀疑了我应该怎么帮他又不暴露自己", "keywords": ["队友被质疑", "保护队友", "转移注意力", "狼队友配合"]},
    {"id": "Q15", "role": "Villager", "phase": "DAY_VOTE", "query_text": "我作为平民不确定该投谁怎么做出正确的投票", "keywords": ["平民投票", "投票决定", "不确定投谁"]},
    {"id": "Q16", "role": "global", "phase": "global", "query_text": "我是新手第一次玩狼人杀需要了解基础规则和术语", "keywords": ["新手", "基础规则", "术语", "游戏规则"]},
    {"id": "Q17", "role": "Witch", "phase": "DAY_SPEECH", "query_text": "我救过一个人银水要不要在白天报出来", "keywords": ["银水", "报银水", "跳身份", "女巫发言"]},
    {"id": "Q18", "role": "Seer", "phase": "NIGHT_SEER_ACTION", "query_text": "今晚我要验一个人应该优先验谁", "keywords": ["查验选择", "验人优先级", "首验", "查验目标"]},
    {"id": "Q19", "role": "global", "phase": "mid_game", "query_text": "我发现之前站错边了现在怎么纠正", "keywords": ["站错边", "修正判断", "站边", "纠正"]},
    {"id": "Q20", "role": "Werewolf", "phase": "DAY_VOTE", "query_text": "关键时刻需要和狼队友统一冲票推掉一个好人", "keywords": ["冲票", "统一投票", "冲票战术", "绑票"]},
]
...
def load_strategies():
    ...
    for sit, rec, rat, role, phase, q in c.fetchall():
        docs.append({"situation": sit or "", "recommended": rec or "", "rationale": rat or "",
...
def eval_method(name, func, queries, warmup=True):
    ...
    for q in queries:
        ...
        metrics.append({
            "id": q["id"], "P@3": p_at_k(results, q, 3), "P@5": p_at_k(results, q, 5),
            "P@10": p_at_k(results, q, 10), "MRR": mrr(results, q),
            "NDCG@5": ndcg_at_k(results, q, 5), "NDCG@10": ndcg_at_k(results, q, 10),
            "latency_ms": lat, "n": len(results),
        })
    return metrics
...
def print_summary(all_metrics, methods):
    ...
    keys = [("P@5", "P@5"), ("P@10", "P@10"), ("MRR", "MRR"),
...
def main():
    ...
    def a_search(q):
        ...
        c.execute("""
            ...
            LIMIT 10
        """, (q["role"], q["phase"]))
        results = [{"situation": r[0] or "", "strategy": r[1] or "", "quality": float(r[2]) if r[2] else 0.8}
                   for r in c.fetchall()]
        conn.close()
        return results
    ...
    def c_search(q):
        ...
        return [{"situation": docs[i]["situation"], "strategy": docs[i]["recommended"], "quality": docs[i]["quality"]}
                for i in top]
    ...
    t0 = time.perf_counter()
    bge_embs = np.asarray(model.encode(bge_texts, normalize_embeddings=True, batch_size=32, show_progress_bar=False), dtype=np.float32)
    enc_t = time.perf_counter() - t0
    print(f"  Model load: {load_t:.1f}s, Encode {len(docs)} docs: {enc_t:.1f}s, Emb shape: {bge_embs.shape}, Memory: {bge_embs.nbytes/1024**2:.1f}MB")

    def d_search(q):
        qe = np.asarray(model.encode(q["query_text"], normalize_embeddings=True), dtype=np.float32)
        sims = np.dot(bge_embs, qe)
        rb = np.array([0.15 if d["role"]==q["role"] else (0.05 if d["role"]=="global" else 0) for d in docs], dtype=np.float32)
        pb = np.array([0.08 if d["phase"]==q["phase"] else (0.03 if d["phase"]=="global" else 0) for d in docs], dtype=np.float32)
    ...
    # Print summary
    all_metrics = {"A_Metadata": metrics_a, "B_FullText": metrics_b,
                   "C_TFIDF": metrics_c, "D_BGE-M3": metrics_d}
    ...
    for i in range(len(TEST_QUERIES)):
        vals = {"A": metrics_a[i]["P@5"], "B": metrics_b[i]["P@5"],
                "C": metrics_c[i]["P@5"], "D": metrics_d[i]["P@5"]}
    ...
    for name in methods:
        ...
        print(f"  {name:12s}: mean={np.mean(lats):.1f}ms p50={np.percentile(lats,50):.1f}ms p95={np.percentile(lats,95):.1f}ms")
