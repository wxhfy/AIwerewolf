...
| 策略使用决策质量差异 | +0.0823 |
| Target-seat Seer paired pipeline pilot | 23 pairs |
| Target-seat Seer adjusted delta | +6.2817 |
| Target-seat Seer role-task delta | +0.0954 |
| Target-seat candidate fallback / invalid | 0 / 0 |

数据口径见 [`docs/evidence/README.md`](docs/evidence/README.md)。这些结果用于展示工程链路、复盘能力和策略回流趋势；target-seat 23-pair pipeline pilot 已跑通且趋势为正，但 `accepted=false`、bootstrap CI 仍跨 0，因此不写成 Track C 因果胜率提升。
