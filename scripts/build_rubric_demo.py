...
from backend.eval.process_score_v3 import (
    ProcessScoreV3Result,
    compute_process_score_v3,
    compute_game_value,
    GameEvaluationValue,
)
...
ROLE_CN_MAP = {
    "Seer": "预言家", "Witch": "女巫", "Guard": "守卫",
    "Hunter": "猎人", "Werewolf": "狼人", "Villager": "村民",
}

MISTAKE_TYPE_CN = {
    "speech": "发言", "vote": "投票", "ability": "技能/夜晚行动",
}
...
def _build_demo_data(states: list[tuple[str, str, Any]]) -> list[DemoGameData]:
    ...
    for source, short_id, state in states:
        ...
        results.append(DemoGameData(
            source=source, game_id_short=short_id, game_id=state.id,
            winner=state.winner.value if state.winner else "unknown",
            total_players=len(state.players), total_events=len(state.events),
            role_setup=role_setup,
            agent_version="fixture-heuristic-v1", model_id="fixture",
            seed=str(getattr(state, "seed", "N/A") or "N/A"),
            published_doc=doc, opportunities=opps,
            process_scores_v3=[], game_value=None,  # type: ignore
    ...
    for i, bc in enumerate(bad_cases):
        ...
        if not matched_cfs:
            for cf in counterfactuals:
                ...
                type_ok = (
                    (bc_type == "speech" and cf_type in ("info_release", "speech")) or
                    (bc_type == "vote" and cf_type == "vote") or
                    (bc_type == "ability" and cf_type == "skill")
                )
                if not type_ok or bc_player not in cf_orig:
                    continue
                pos = cf_orig.find(bc_player)
                before = cf_orig[:pos].lower()
                victim_kw = ["poisoned", "shot", "killed", "died", "victim",
                            "误伤", "毒杀", "枪杀", "杀死", "刀了", "毒了", "target", "targeted"]
        ...
        if missing:
            ...
            if synth_cf:
                ...
                matched_cfs.append({"_synthetic": True, "_synth_key": synth_cf["counterfactual_type"] + "_" + bc_player})
                missing = False
                missing_reason = ""
            else:
...
def _synthesize_counterfactual(bc: dict, game_id_short: str) -> dict | None:
    ...
    if bc_role == "Guard" and bc_type == "ability" and ("连续" in bc_desc or "consecutive" in bc_desc.lower()):
        return {
            ...
            "alternative_decision": f"轮换守护目标：保护当前公共压力最高的好人角色，或根据狼刀逻辑预判最可能被刀的目标。",
    ...
    if bc_role == "Werewolf" and bc_type == "vote" and "队友" in bc_desc:
        return {
            ...
            "day": bc_day,
            "original_decision": bc_desc,
            "alternative_decision": f"协调狼队投票集中在一个高价值好人目标上，或将投队友行为包装为公开的倒钩策略以降低自身嫌疑。",
...
def _build_improvement_suggestions(
    ...
    for review in player_reviews:
        ...
        if cd.get("_ambiguous", False):
            ...
            training_focus = review_suggestions[0] if review_suggestions else (
                f"{ROLE_CN_MAP.get(role, role)}角色专项训练"
            )
...
def _build_markdown_report(games: list[DemoGameData], summary: dict) -> str:
    ...
    # ---- Rubric Alignment ----
    has_speech = any(
        op.get("opportunity_type") in ("speech", "seer_release")
        for g in games for op in g.opportunities
    )
    has_vote = any(
        op.get("opportunity_type") == "vote"
        for g in games for op in g.opportunities
    )
    has_skill = any(
        op.get("opportunity_type") in ("seer_check", "witch_save", "witch_poison",
                                        "guard_protect", "hunter_shot", "werewolf_kill")
        for g in games for op in g.opportunities
    )
    ...
    lines.extend([
        ...
        f"| Leaderboard | 单局 demo 不适用 | **PENDING** |",
        "",
    ])
...
def main():
    ...
    print(f"  Rubric: scoring=PASS review=PASS cf={'PASS' if cd_info['missing_counterfactual'] == 0 else 'PARTIAL'} report=PASS leaderboard=PENDING")
    print(f"\n  输出文件:")
