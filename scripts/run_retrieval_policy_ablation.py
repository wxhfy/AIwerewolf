...
def run_ablation(
    ...
    # Save results
    evidence_status = _ablation_evidence_status(all_stats)
    results = {
        ...
        "claim_boundary": (
            "complete_online_ablation can be used as run-level evidence; "
            "incomplete_online_ablation is framework output only and must not be cited as a policy uplift conclusion."
        ),
        "policies": {
            p: {
                "policy": p,
...
def _ablation_evidence_status(all_stats: Dict[str, PolicyStats]) -> str:
    """Return whether this run has completed online evidence for every policy."""
    if any(policy not in all_stats for policy in POLICIES):
        return "incomplete_online_ablation"
    for policy in POLICIES:
        stats = all_stats[policy]
        if stats.n_games <= 0 or stats.n_errors > 0 or stats.n_completed < stats.n_games:
            return "incomplete_online_ablation"
    return "complete_online_ablation"
...
def _build_ablation_summary(
    ...
    lines = [
        ...
        "",
        (
            "**Claim boundary**: this report is framework output only until every policy has "
            "completed online game runs without errors. Do not cite incomplete output as a Track C uplift conclusion."
            if evidence_status != "complete_online_ablation"
            else "**Claim boundary**: completed online ablation output; still review sample size, confidence intervals, and fallback gates before citing uplift."
        ),
        "",
        "## Policy Comparison",
        "",
    ...
    if evidence_status != "complete_online_ablation":
        lines += [
            "No policy recommendation is made because the online game execution is incomplete.",
            "Complete the game-run wiring and rerun this ablation before using it as comparative evidence.",
        ]
    else:
        completed_stats = [stats for stats in all_stats.values() if stats.n_completed > 0]
        best = max(completed_stats, key=lambda s: s.avg_process_score)
        lines += [
            f"Highest observed average process score in this run: **`{best.policy}`**.",
            "Treat this as a candidate recommendation only after checking confidence intervals, fallback counts, and invalid actions.",
        ]

    lines += ["", "---", "*Generated by scripts/run_retrieval_policy_ablation.py*"]
...
if __name__ == "__main__":
    ...
    ap.add_argument("--games", type=int, default=5, help="Games per policy group")
