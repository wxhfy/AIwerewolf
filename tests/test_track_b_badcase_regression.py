...
from backend.eval.review import MetricsCalculator, ReviewReportBuilder
...
def _make_player(pid: str, name: str, role: Role, *, alive: bool = True) -> Player:
    return Player(
        id=pid, seat=int(pid[1:]), name=name, role=role,
...
def _make_vote(day: int, voter: Player, target: Player) -> GameEvent:
    return GameEvent.create(
        day=day, phase=Phase.DAY_VOTE, type=EventType.VOTE_CAST,
        visibility="public",
        payload={
...
def _make_speech(day: int, actor: Player, speech: str, *, phase: Phase = Phase.DAY_SPEECH) -> GameEvent:
    return GameEvent.create(
        day=day, phase=phase, type=EventType.CHAT_MESSAGE,
        visibility="public",
        payload={
...
def _make_night_action(
    day: int, actor: Player, action_type: str, target: Player,
    *, phase: Phase = Phase.NIGHT_WOLF_ACTION,
) -> GameEvent:
    return GameEvent.create(
        day=day, phase=phase, type=EventType.NIGHT_ACTION,
        visibility="private",
        payload={
            "actor_id": actor.id, "actor_name": actor.name,
            "action_type": action_type, "target_id": target.id,
        },
        visible_to=[actor.id],
    )
...
def _make_seer_result(day: int, seer: Player, target: Player, *, is_wolf: bool) -> GameEvent:
    return GameEvent.create(
        day=day, phase=Phase.NIGHT_SEER_ACTION, type=EventType.PRIVATE_INFO,
        visibility="private",
        payload={
            "kind": "seer_result", "target_id": target.id,
...
def _make_witch_info(day: int, witch: Player, victim: Player) -> GameEvent:
    ...
    return GameEvent.create(
        day=day, phase=Phase.NIGHT_WITCH_ACTION, type=EventType.PRIVATE_INFO,
        visibility="private",
        payload={
            "kind": "witch_victim", "target_id": victim.id,
...
def _make_wolf_tally(day: int, target: Player, votes: dict[str, str]) -> GameEvent:
    ...
    return GameEvent.create(
        day=day, phase=Phase.NIGHT_WOLF_ACTION, type=EventType.PRIVATE_INFO,
        visibility="private",
        payload={
            "kind": "wolf_attack_tally", "target_id": target.id,
...
def _make_death(day: int, player: Player, reason: str, *, phase: Phase | None = None) -> GameEvent:
    ...
    return GameEvent.create(
        day=day, phase=phase, type=EventType.PLAYER_DIED,
        visibility="public",
        payload={
...
def _make_hunter_shot(day: int, hunter: Player, target: Player) -> GameEvent:
    return GameEvent.create(
        day=day, phase=Phase.HUNTER_SHOOT, type=EventType.HUNTER_SHOT,
        visibility="public",
        payload={
            "hunter_id": hunter.id, "target_id": target.id,
...
def build_badcase_001_fixture() -> GameState:
    ...
    events = [
        # N1
        n1_guard, n1_wolf_tally, n1_wolf1_kill, n1_wolf2_kill,
        n1_witch_info, n1_witch_save,
        n1_seer_result, n1_seer_check,
        # D1 speeches
        d1_speech_p3, d1_speech_p4, d1_speech_p5, d1_speech_p6,
        d1_speech_p7, d1_speech_p1, d1_speech_p2,
        # D1 votes
        d1_vote_p1, d1_vote_p2, d1_vote_p3, d1_vote_p4,
        d1_vote_p5, d1_vote_p6, d1_vote_p7,
        # D1 resolve
        d1_death_p6, d1_hunter_shot, d1_death_p3,
        # N2
        n2_guard, n2_wolf_tally, n2_wolf1_kill, n2_wolf2_kill,
        n2_witch_info, n2_witch_poison,
        # N2 resolve
        n2_death_p4, n2_death_p5,
    ]

    # --- Decision records for opportunity extraction ---
    decisions = [
        # N1
        _make_decision(game_id, p5, Role.GUARD, 1, "NIGHT_GUARD", "GUARD",
                       {"type": "guard", "target_id": "P5", "reasoning": "首夜自守是标准操作"}),
        _make_decision(game_id, p1, Role.WEREWOLF, 1, "NIGHT_WOLF", "KILL",
                       {"type": "attack", "target_id": "P7", "reasoning": "刀村民减少好人数量"}),
        _make_decision(game_id, p2, Role.WEREWOLF, 1, "NIGHT_WOLF", "KILL",
                       {"type": "attack", "target_id": "P7", "reasoning": "跟队友一致刀村民"}),
        _make_decision(game_id, p4, Role.WITCH, 1, "NIGHT_WITCH", "WITCH",
                       {"type": "save", "target_id": "P7", "reasoning": "首夜救人保平安"}),
        _make_decision(game_id, p3, Role.SEER, 1, "NIGHT_SEER", "DIVINE",
                       {"type": "divine", "target_id": "P1", "reasoning": "查验 P1 摸身份"}),
        # D1 speeches
        _make_decision(game_id, p3, Role.SEER, 1, "DAY_SPEECH", "TALK",
                       {"type": "speech", "speech": "我这轮没什么信息，先随便听听吧。我感觉 P6 猎人有点紧张，今天可以先出 P6。"},
                       observation={"private": "昨晚查验 P1 是狼人"}),
        _make_decision(game_id, p4, Role.WITCH, 1, "DAY_SPEECH", "TALK",
                       {"type": "speech", "speech": "我知道昨晚 P7 被刀了，但我不想说为什么。今天我觉得 P5 怪怪的，先盯 P5。"},
                       observation={"private": "昨晚 P7 被刀，我用了解药"}),
        _make_decision(game_id, p5, Role.GUARD, 1, "DAY_SPEECH", "TALK",
                       {"type": "speech", "speech": "我是守卫，我昨晚守自己。今晚我还会继续守自己，你们不用管我。"}),
        _make_decision(game_id, p6, Role.HUNTER, 1, "DAY_SPEECH", "TALK",
                       {"type": "speech", "speech": "你们爱出就出，我反正有身份。谁投我我就带谁。"}),
        _make_decision(game_id, p7, Role.VILLAGER, 1, "DAY_SPEECH", "TALK",
                       {"type": "speech", "speech": "我觉得 P1 说得挺有道理，P6 确实像狼。今天就先出 P6 吧。"}),
        _make_decision(game_id, p1, Role.WEREWOLF, 1, "DAY_SPEECH", "TALK",
                       {"type": "speech", "speech": "P6 发言太紧张了，明显有问题。今天我归票 P6，好人们跟我投。"}),
        _make_decision(game_id, p2, Role.WEREWOLF, 1, "DAY_SPEECH", "TALK",
                       {"type": "speech", "speech": "我也觉得 P6 不对劲，跟 P1 投 P6。"}),
        # D1 votes
        _make_decision(game_id, p1, Role.WEREWOLF, 1, "DAY_VOTE", "VOTE",
                       {"type": "vote", "target_id": "P6", "reasoning": "推猎人出局"}),
        _make_decision(game_id, p2, Role.WEREWOLF, 1, "DAY_VOTE", "VOTE",
                       {"type": "vote", "target_id": "P6", "reasoning": "跟队友推猎人"}),
        _make_decision(game_id, p3, Role.SEER, 1, "DAY_VOTE", "VOTE",
                       {"type": "vote", "target_id": "P6", "reasoning": "感觉 P6 像狼"},
                       observation={"private": "我知道 P1 是狼但没有公开"}),
        _make_decision(game_id, p4, Role.WITCH, 1, "DAY_VOTE", "VOTE",
                       {"type": "vote", "target_id": "P5", "reasoning": "P5 怪怪的"}),
        _make_decision(game_id, p5, Role.GUARD, 1, "DAY_VOTE", "VOTE",
                       {"type": "vote", "target_id": "P6", "reasoning": "跟风投票"}),
        _make_decision(game_id, p6, Role.HUNTER, 1, "DAY_VOTE", "VOTE",
                       {"type": "vote", "target_id": "P1", "reasoning": "P1 带头踩我"}),
        _make_decision(game_id, p7, Role.VILLAGER, 1, "DAY_VOTE", "VOTE",
                       {"type": "vote", "target_id": "P6", "reasoning": "跟 P1 节奏"}),
        # D1 hunter shot
        _make_decision(game_id, p6, Role.HUNTER, 1, "NIGHT_HUNTER_SHOOT", "SHOOT",
                       {"type": "shoot", "target_id": "P3", "reasoning": "P3 也投我，带走"}),
        # N2
        _make_decision(game_id, p5, Role.GUARD, 2, "NIGHT_GUARD", "GUARD",
                       {"type": "guard", "target_id": "P5", "reasoning": "继续守自己"}),
        _make_decision(game_id, p1, Role.WEREWOLF, 2, "NIGHT_WOLF", "KILL",
                       {"type": "attack", "target_id": "P4", "reasoning": "刀女巫，去神职"}),
        _make_decision(game_id, p2, Role.WEREWOLF, 2, "NIGHT_WOLF", "KILL",
                       {"type": "attack", "target_id": "P4", "reasoning": "跟队友刀女巫"}),
        _make_decision(game_id, p4, Role.WITCH, 2, "NIGHT_WITCH", "WITCH",
...
def test_badcase_001_scores_are_separated(badcase_001_state: GameState) -> None:
    ...
    for pid in sorted(scores_by_id, key=lambda x: int(x[1:])):
        ps = scores_by_id[pid]
        print(f"  {pid} ({ps.player_name:8s} {ps.role:10s}): "
    ...
    assert scores_by_id["P3"].process_score <= 45, \
        f"P3 Seer process_score too high: {scores_by_id['P3'].process_score}"
...
def test_badcase_001_detects_seer_withheld_wolf_check(badcase_001_state: GameState) -> None:
    ...
    seer_bad = [bc for bc in bad_cases
                if bc.player_name == "预言家A" and "speech" in bc.mistake_type]
    assert seer_bad, \
        f"Expected seer withheld-wolf-check bad case, got: {descriptions}"
...
def test_badcase_001_detects_witch_poisoned_village_power(badcase_001_state: GameState) -> None:
    ...
    witch_poison_bad = [
        bc for bc in bad_cases
        if bc.player_name == "女巫A"
        and bc.mistake_type == "ability"
        and "poison" in bc.description.lower()
    ]
    assert witch_poison_bad, \
        f"Expected witch poison village bad case, got: {[bc.description for bc in bad_cases]}"
...
def test_badcase_001_detects_hunter_shot_key_village_power(badcase_001_state: GameState) -> None:
    ...
    hunter_shot_bad = [
        bc for bc in bad_cases
        if bc.player_name == "猎人A"
        and bc.mistake_type == "ability"
        and "shot" in bc.description.lower()
    ]
    assert hunter_shot_bad, \
        f"Expected hunter shot village bad case, got: {[bc.description for bc in bad_cases]}"
...
def test_badcase_001_detects_guard_consecutive_same_target(badcase_001_state: GameState) -> None:
    ...
    guard_repeat_bad = [
        bc for bc in bad_cases
        if bc.player_name == "守卫A"
        and bc.mistake_type == "ability"
        and "repeated" in bc.description.lower()
    ]
...
def test_badcase_001_detects_seer_voted_against_checked_wolf(badcase_001_state: GameState) -> None:
    ...
    seer_speech_bad = [
        bc for bc in bad_cases
        if bc.player_name == "预言家A" and bc.mistake_type == "speech"
    ]
    assert seer_speech_bad, \
        f"Expected seer withheld check speech bad case, got: {[bc.description for bc in bad_cases]}"

    # 2. Seer voted against known wolf (NEW rule)
    seer_vote_bad = [
        bc for bc in bad_cases
        if bc.player_name == "预言家A" and bc.mistake_type == "vote"
    ]
...
def test_badcase_001_wolves_score_high_for_good_play(badcase_001_state: GameState) -> None:
    ...
    assert scores_by_id["P1"].vote_score >= 0.7, \
        f"P1 vote_score too low: {scores_by_id['P1'].vote_score}"
    assert scores_by_id["P2"].vote_score >= 0.7, \
        f"P2 vote_score too low: {scores_by_id['P2'].vote_score}"

    # Wolf skill score should be reasonable (killed villagers then witch)
    assert scores_by_id["P1"].skill_score >= 0.5, \
        f"P1 skill_score too low: {scores_by_id['P1'].skill_score}"

    # No bad cases for wolves
    bad_cases = metrics.metadata.get("bad_case_reports", [])
    wolf_bad = [bc for bc in bad_cases if "狼人" in bc.player_name]
    assert len(wolf_bad) == 0, \
        f"Wolves should not have bad cases, got: {[bc.description for bc in wolf_bad]}"
...
def test_badcase_001_opportunities_extracted(badcase_001_opportunities) -> None:
    ...
    for op in opps:
        print(f"  {op.opportunity_id[:50]:50s} {op.player_id} {op.opportunity_type:20s} "
              f"day={op.day} role={op.role}")

    expected_types = {"guard_protect", "werewolf_kill", "witch_save",
                      "witch_poison", "seer_check", "hunter_shot",
                      "vote", "speech"}
...
def test_badcase_001_process_scores_from_opportunities(badcase_001_opportunities) -> None:
    ...
    for pid in sorted(scores_by_id, key=lambda x: int(x[1:])):
        r = scores_by_id[pid]
        print(f"  {pid} ({r.role:10s}): process={r.process_score:.4f} "
    ...
    for pid in ["P1", "P2", "P3", "P4", "P5", "P6", "P7"]:
        ...
        assert scores_by_id[pid].num_opportunities >= 1, \
            f"{pid} has no opportunities"
...
def test_badcase_001_seer_release_opportunity_low(badcase_001_opportunities) -> None:
    ...
    """
    seer_speech_opps = [
        op for op in badcase_001_opportunities
        if op.player_id == "P3" and op.day == 1
        and op.opportunity_type in ("speech", "seer_release")
    ]
    ...
    for op in seer_speech_opps:
        ...
        print(f"  P3 speech opp: type={op.opportunity_type}, "
              f"private_ctx={op.private_context_summary[:80]}")
...
def test_badcase_001_guard_consecutive_opportunities(badcase_001_opportunities) -> None:
    ...
    guard_opps = [
        op for op in badcase_001_opportunities
        if op.player_id == "P5" and op.opportunity_type == "guard_protect"
    ]
    assert len(guard_opps) >= 2, \
        f"Expected >= 2 guard opportunities, got {len(guard_opps)}"
    ...
    assert len(p5_targets) >= 2, \
        f"Expected both guards to target P5, got targets: {targets}"
