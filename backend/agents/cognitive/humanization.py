...
def build_humanization_profile(
    ...
    if persona is None and mind is None:
        return HumanizationProfile(
            vote_temperature=0.8, suspicion_gain=1.0, recency_weight=1.0,
            grudge_weight=0.8, follow_weight=0.6, stubbornness=0.5,
            self_protection_weight=1.0, speech_min_segments=2, speech_max_segments=3,
        )
    ...
    if m.courage == "bold":
        vote_temperature = 0.3; grudge_weight = 1.5; self_protection_weight = 0.5
    elif m.courage == "cautious":
        vote_temperature = 1.2; grudge_weight = 0.4; self_protection_weight = 1.5
    else:
        vote_temperature = 0.6; grudge_weight = 0.8; self_protection_weight = 1.0

    # ---- Memory bias → recency + stubbornness ----
    if m.memory_bias == "recent":
        recency_weight = 2.0; stubbornness = 0.2
    elif m.memory_bias == "first_impression":
        recency_weight = 0.4; stubbornness = 1.5
    elif m.memory_bias == "selective":
        recency_weight = 1.0; stubbornness = 1.0
    else:
        recency_weight = 0.8; stubbornness = 0.5
    ...
    if m.table_presence == "dominant":
        speech_max = max(speech_max, 4); speech_min = 2; follow_weight = 0.3
    elif m.table_presence == "quiet":
        speech_max = min(speech_max, 2); speech_min = 1; follow_weight = 0.5
    else:
        speech_max = min(speech_max, 3); speech_min = 2; follow_weight = 0.6

    # ---- Style label adjustments ----
    style = p.style_label or ""
    if style in ("aggressive", "provocative"):
        grudge_weight *= 1.5; vote_temperature = max(0.15, vote_temperature - 0.2)
    elif style in ("analytical", "meticulous"):
        suspicion_gain *= 0.8; speech_max = max(speech_max, 3)
    elif style == "observant":
        speech_max = min(speech_max, 2); speech_min = min(speech_min, 1)
