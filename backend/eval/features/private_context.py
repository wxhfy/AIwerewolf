...
class PrivateContextFeatures:
    ...
    def extract(self, opportunity: dict[str, Any], context: dict[str, Any] | None = None) -> dict[str, float | int | str]:
        ...
        feats.update(self._wolf_features(is_wolf, role, op_type, speech_text,
                                          target_id, known_wolf_ids, target_feat, game_feat, player_id))
        return feats

    def _wolf_features(self, is_wolf, role, op_type, speech_text, target_id,
                       known_wolf_ids, target_feat, game_feat, player_id) -> dict:
        feats: dict[str, float | int | str] = {}
        ...
        if is_wolf and op_type == "vote" and target_id:
            ...
            if target_is_known:
                ...
                if tr in ("Seer", "Witch"): vf = max(0.0, vf - 0.3)
                elif tr in ("Guard", "Hunter"): vf = max(0.0, vf - 0.1)
        ...
        if op_type == "werewolf_kill":
            ...
            if "预言家" in str(ke) or "Seer" in str(ke): bv = max(bv, 0.95)
            if "女巫" in str(ke) or "Witch" in str(ke): bv = max(bv, 0.90)
            if "守卫" in str(ke) or "Guard" in str(ke): bv = max(bv, 0.80)
            if not ke: bv = max(bv, 0.5)
        ...
        if op_type in ("speech", "seer_release") and speech_text:
            refs = ["查验", "查杀", "金水", "投票", "票型", "发言", "昨天", "刚才", "这轮", "上轮",
                    "P1", "P2", "P3", "P4", "P5", "P6", "P7", "因为", "所以", "证据", "逻辑"]
        ...
        gc = 0.0
        if is_wolf:
            if op_type == "speech" and feats.get("wolf_perspective_leak_score", 0) > 0.3: gc = 0.7
            if op_type == "vote" and feats.get("vote_coordination_failure", 0) > 0.3: gc = max(gc, 0.5)
            if op_type == "werewolf_kill" and feats.get("counterfactual_target_gap", 0) > 0.5: gc = max(gc, 0.6)
        if role == "Seer" and feats.get("private_info_withheld", 0): gc = 0.8
        if role == "Witch" and op_type == "witch_poison" and feats.get("target_is_village", 0) == 1: gc = 0.9
        if role == "Hunter" and op_type == "hunter_shot" and feats.get("target_is_village", 0) == 1: gc = 0.9
        ...
        if op_type in ("speech", "seer_release", "vote"):
            ...
            if is_wolf and feats.get("teammate_overprotection", 0) > 0.5: el = 0.7
            if gg > 0.2: el = max(el, 0.4)
            if feats.get("voted_elsewhere_despite_known_wolf", 0): el = max(el, 0.8)
