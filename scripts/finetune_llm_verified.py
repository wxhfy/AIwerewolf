...
def generate_verified_triplets(docs, n_samples=300, verify_ratio=0.5):
    ...
    print(f"  Saved to data/ft_llm_verified.jsonl")
...
class TripletDataset(Dataset):
    ...
    def __getitem__(self, idx):
        t = self.triplets[idx]
        return {"anchor": t["anchor_text"], "positive": t["positive_text"],
...
def evaluate_model(model_path, docs, queries, label=""):
    ...
    for q in queries:
        ...
        for k in [5, 10]:
            kk = min(k, len(res))
            rel = [1 if sum(1 for kw in q["kw"] if kw.lower() in f"{r['situation']} {r['strategy']}".lower()) > 0 else 0 for r in res[:kk]]
