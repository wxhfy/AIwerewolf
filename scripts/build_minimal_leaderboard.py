...
def build_report(games: list[GameResult], leaderboard: dict) -> str:
    ...
    for g in games[:4]:  # Show first 4 games
        ...
        try:
            ...
            agents = create_agents(players, config)
            game = WerewolfGame(
                players=players, agents=agents, seed=g.seed, max_days=MAX_DAYS, strategy_version=g.agent_version
            )
            game.play()
            state = game.state
        except Exception:
    ...
    if len(entries) >= 2:
        ...
        if critical_diff > 0:
            lines.append(
                f"- 关键错误率高出 {critical_diff}（{second['critical_mistake_rate']} vs {first['critical_mistake_rate']}）"
            )
...
async def main_async():
    ...
    print("  Rubric Leaderboard: PASS_SMOKE")
