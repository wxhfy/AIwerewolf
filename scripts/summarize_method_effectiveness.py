...
def build_evidence(
    *,
    ...
    default_policy = retrieval_metrics.get("same_role_all_mbti", {})
    baseline_policy = retrieval_metrics.get("global_only", {})
    exact_policy = retrieval_metrics.get("same_role_same_mbti", {})
    role_default_rows = [row for row in per_role_rows if row.get("policy") == "same_role_all_mbti"]
...
def render_report(evidence: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "可追溯性说明：本报告引用的 `docs/experiments/` 和 `outputs/` 原始产物是本地实验输出，按仓库规则不进入 GitHub；机器可读摘要会汇总到本地 ignored 目录 `docs/evidence/`。",
        "",
        "## 1. 结论摘要",
        "",
        ...
        "## 5. 单角色检索有效性",
        "",
        "单角色检索的详细机制、分角色命中率和语料池规模可生成到本地 ignored 目录 `docs/evidence/`。",
        "",
