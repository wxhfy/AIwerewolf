...
def run_post_game_scoring(game_state: Any, game_id: str) -> int:
    ...
    """
    try:
        from backend.eval.per_step_scorer import PerStepScorer, ScoredStep, PlayerReviewReport
    ...
    try:
        ...
        if getattr(llm_client, "available", True):
            ...
            logger.info(
                ...
                f"Tier2=light_llm (enabled), Tier3=heavy_llm (enabled)"
            )
        else:
    ...
    for s in scores:
        ...
        by_player[s.player_id].append(ScoredStep(
...
def build_speech_acts_from_decisions(decision_dicts: list[dict]) -> list[dict]:
    ...
    for d in decision_dicts:
        ...
        has_self_defense = bool(re.search(
