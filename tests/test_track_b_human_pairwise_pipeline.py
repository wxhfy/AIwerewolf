...
class TestHumanPairwisePipeline:
    def test_build_human_pairwise_queue_runs(self):
        rc, stdout, stderr = _run([sys.executable, "scripts/build_human_pairwise_queue.py", "--max", "10"])
    ...
    def test_human_pairwise_queue_schema(self):
        ...
        if not qpath.exists():
            subprocess.run(
                [sys.executable, "scripts/build_human_pairwise_queue.py", "--max", "10"],
                capture_output=True,
                timeout=120,
            )
        if not qpath.exists():
    ...
    def test_human_pairwise_label_sample_validates(self):
        ...
        rc, stdout, stderr = _run(
            [
                ...
                str(spath),
                "--output",
                str(DATA / "human_pairwise_validation_result.json"),
            ]
        )
