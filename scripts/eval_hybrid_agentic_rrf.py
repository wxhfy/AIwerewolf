...
TEST_QUERIES = [
    {"id":"G1","role":"Werewolf","phase":"DAY_SPEECH",
     "text":"3号狼人被2号预言家查杀，需要伪装好人发言同时保护被怀疑的7号狼队友",
     "kw":["被查杀","伪装好人","保护队友","狼人发言"]},
    {"id":"G2","role":"Witch","phase":"NIGHT_WITCH_ACTION",
     "text":"女巫第一晚，7号被刀，解药毒药都在，决定是否救人",
     "kw":["首夜解药","第一晚救人","解药使用"]},
    {"id":"G3","role":"Seer","phase":"DAY_BADGE_SPEECH",
     "text":"5号预言家查验3号是狼人(查杀)，需要竞选警长报警徽流",
     "kw":["警徽流","警长竞选","查杀","预言家发言"]},
    {"id":"G4","role":"Villager","phase":"DAY_VOTE",
     "text":"1号平民面对两个预言家对跳，需要决定投票站边",
     "kw":["平民投票","对跳预言家","站边"]},
    {"id":"G5","role":"Guard","phase":"NIGHT_GUARD_ACTION",
     "text":"10号守卫上一晚守了预言家，女巫解药已用，需要选择守护目标",
     "kw":["守护目标","轮换守护","不能连续守同一人"]},
    {"id":"G6","role":"WhiteWolfKing","phase":"DAY_SPEECH",
     "text":"白狼王6号，真预言家已坐实，狼队友7号即将被票，是否自爆带走预言家",
     "kw":["白狼王自爆","自爆时机","吞警徽"]},
    {"id":"G7","role":"Hunter","phase":"DAY_SPEECH",
     "text":"4号猎人被多人投票即将出局，3号和7号带头推你，需要留枪徽流威慑狼人",
     "kw":["猎人跳身份","枪徽流","开枪威慑"]},
    {"id":"G8","role":"Werewolf","phase":"NIGHT_WOLF_ACTION",
    ...
    def __init__(self, docs):
        ...
        for i, d in enumerate(docs):
            for w in set(jieba.cut(f"{d['situation']} {d['strategy']} {d['rationale']}")):
                if len(w) >= 2: self._idx.setdefault(w, set()).add(i)
        ...
        for kw in keywords:
            for token in jieba.cut(str(kw)):
                if len(token) >= 2 and token in self._idx:
                    for i in self._idx[token]: scores[i] += 1
...
class RRFEngine:
    def __init__(self, docs):
        ...
        self.dembs = np.asarray(self.model.encode(dtx, normalize_embeddings=True, batch_size=32, show_progress_bar=False), dtype=np.float32)
    ...
    def search(self, text, role="", phase="", topk=10):
        ...
        ds = np.dot(self.dembs, qe) + np.array([0.12 if d["role"]==role else (0.03 if d["role"]=="global" else 0) for d in self.docs], dtype=np.float32)
        ds += np.array([0.06 if d["phase"]==phase else (0.02 if d["phase"]=="global" else 0) for d in self.docs], dtype=np.float32)
    ...
    def rerank_subset(self, text, candidate_indices, role="", phase="", topk=10):
        ...
        for i in candidate_indices:
            b_score = bs[i] + (0.1 if self.docs[i]["role"]==role else (0.03 if self.docs[i]["role"]=="global" else 0))
            d_score = ds[i] + (0.12 if self.docs[i]["role"]==role else (0.03 if self.docs[i]["role"]=="global" else 0))
            # RRF within subset
...
返回JSON: {{"winner":"A/B/tie","score_a":1-5,"score_b":1-5,"reasoning":"..."}}"""
    ...
    except: return {"winner":"tie","score_a":3,"score_b":3}
...
if __name__ == "__main__":
    ...
    all_winners = {m: 0 for m in methods}
    ...
    for qi, q in enumerate(TEST_QUERIES):
        ...
        for compare_to, target in [("C_Hybrid", "B_RRF"), ("D_Parallel", "B_RRF")]:
            if compare_to in results and target in results:
                ...
                print(f"  {compare_to} vs {target}: winner={winner} "
