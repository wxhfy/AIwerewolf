...
def test_integration_report_exists():
    path = ROOT / "docs" / "track_b_speech_semantic_audit_integration_report.md"
    if not path.exists():
        pytest.skip("Speech semantic integration report is an optional local report artifact")


def test_classifier_report_exists():
    path = ROOT / "docs" / "track_b_speech_act_classifier_v0_report.md"
    if not path.exists():
        pytest.skip("Speech act classifier report is an optional local report artifact")
