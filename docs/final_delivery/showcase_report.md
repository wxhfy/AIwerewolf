...
| 口径 | 本报告写法 | 不采用写法 |
| --- | --- | --- |
| Track B 覆盖 | 9,376 非 fake 对局、170,399 逐决策评分 | 把模型榜分组行写成唯一对局数 |
| 模型榜 | 23 唯一对局、161 席位、11 模型 | 泛化为全部历史对局 |
| Track C A/B | 固定场景同角色检索综合分 +0.80 | 写成全局胜率提升 |
| 策略关联 | 使用策略决策评分高出 +0.082 | 写成随机对照实验结论 |
