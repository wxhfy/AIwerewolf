...
| **LLM** | ✅ Verified | 客户端可用（doubao ep-20260514115354-k4jz4），health check JSON 返回 `ok`，同步调用 `chat_sync()` 返回合法响应 | client unavailable 或 health check 失败，或响应为空 | `create_client().chat_sync()` | - |
