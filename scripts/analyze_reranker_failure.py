...
import numpy as np, jieba, time, re
import psycopg2
...
norms = np.linalg.norm(dense_embs, axis=1, keepdims=True); norms[norms==0]=1; dense_embs/=norms
...
test_queries = [
    ...
    {"text": "白狼王什么时候自爆带走关键好人最合适", "role": "WhiteWolfKing", "phase": "DAY_SPEECH", "kw": ["白狼王自爆", "自爆时机"]},
    {"text": "我是平民需要分析局势给出有价值的发言", "role": "Villager", "phase": "DAY_SPEECH", "kw": ["平民发言", "发言质量"]},
    {"text": "今晚狼队应该优先刀谁", "role": "Werewolf", "phase": "NIGHT_WOLF_ACTION", "kw": ["刀人优先级", "刀法"]},
]

def rrf_fuse(b_list, d_list, k=20):
    scores = {}
    for rank, r in enumerate(b_list, 1): scores[int(r)] = scores.get(int(r), 0) + 1/(60+rank)
    for rank, r in enumerate(d_list, 1): scores[int(r)] = scores.get(int(r), 0) + 1/(60+rank)
...
for qi, q in enumerate(test_queries):
    ...
    if rrf_avg > rerank_avg: total_rrf_better += 1
    if rerank_avg > rrf_avg: total_rerank_better += 1
    if grep_avg >= rrf_avg: total_grep_better += 1
    ...
    print(f"  Reranker scores: min={rerank_scores.min():.3f} max={rerank_scores.max():.3f} "
          f"std={rerank_scores.std():.3f}")
...
print(f"  → Reranker {'favors' if corr > 0.1 else 'is neutral to' if abs(corr) < 0.1 else 'disfavors'} longer documents")
...
print(f"  Grep (jieba tokenize + string match):")
...
print(f"  Grep raw (no ranking, just count matches):")
print(f"    P@5={agg2['P@5']:.3f} P@10={agg2['P@10']:.3f} MRR={agg2['MRR']:.3f} "
...
print(f"{'Grep (jieba+count)':<25} {agg['NDCG@5']:>8.3f} {agg['P@5']:>8.3f} {agg['MRR']:>8.3f} {np.mean([m['latency_ms'] for m in metrics]):>7.0f}ms")
if metrics3:
...
print(f"Example: '银水' vs '被救的人' → grep misses, BM25+Dense RRF catches.")
print(f"Reranker degrades because it's a GENERAL cross-encoder that doesn't understand")
print(f"werewolf-specific terminology as well as the domain-tuned BM25+jieba combination.")
