...
TEST_SETS["special"] = [
    ...
    {"id": "E05", "role": "global", "phase": "global", "text": "怎样才能赢", "kw": ["胜利条件", "获胜", "策略"]},
    {
        ...
        "role": "Seer",
        "phase": "DAY_BADGE_SPEECH",
        "text": "我是预言家有查杀但对跳也出现了警徽流怎么安排女巫工作",
        "kw": ["对跳", "警徽流", "查杀", "女巫"],
    },
    {
        "id": "E07",
    ...
    {"id": "E12", "role": "Werewolf", "phase": "global", "text": "自刀", "kw": ["自刀", "狼人"]},
    {
        ...
        "role": "global",
        "phase": "mid_game",
        "text": "预言家被刀女巫没救猎人被票开枪带走了狼现在场上剩1狼2民1神怎么投",
        "kw": ["残局", "轮次", "投票"],
    },
    {
        "id": "E14",
...
class BM25Searcher:
    ...
    def search(self, query, role="", phase="", k=20):
        ...
        scores = scores + np.array(
            [0.1 if d["role"] == role else (0.03 if d["role"] == "global" else 0) for d in self.docs]
        )
        scores = scores + np.array(
            [0.05 if d["phase"] == phase else (0.02 if d["phase"] == "global" else 0) for d in self.docs]
        )
...
def main():
    ...
    methods = {
        ...
        "D_Hybrid_RRF+Rerank": (
            "RRF+Rerank",
            lambda q: reranker.rerank(q["text"], hybrid.search(q["text"], q["role"], q["phase"], k=20), k=10),
        ),
    }
    ...
    for set_name, queries in TEST_SETS.items():
        ...
        for mname, (mlabel, mfunc) in methods.items():
            ...
            print(
                f"  {mlabel:<22s} P@5={agg['P@5']:.3f}  P@10={agg['P@10']:.3f}  "
    ...
    print(
        f"  {'OVERALL':<20} {overall_c:>10.3f} {overall_d:>10.3f} {'+' if overall_d > overall_c else ''}{overall_d - overall_c:>9.3f}"
    )
    ...
    for i, q in enumerate(special_queries):
        ...
        text = q["text"][:42]
        print(
            f"  {q['id']:<6} {text:<45} {pa:>5.2f}{markers[0]} {pb:>5.2f}{markers[1]} {pc:>5.2f}{markers[2]} {pd:>5.2f}{markers[3]}"
        )
