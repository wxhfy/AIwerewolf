...
DEFAULT_POLICY = "same_role_all_mbti"
    ...
    return {
        ...
        "retrieval_pipeline": [
            ...
            },
            {
                "step": 3,
                "name": "关键词召回",
                "description": "优先使用 Agent 给出的关键词/正则在 situation、strategy、rationale 等字段中加权 grep；只有完全没有关键词命中时才回退 BM25 全文检索，避免为凑满 Top-K 注入泛策略。",
                "code": "backend/agents/cognitive/retrieval_prod.py",
            },
            {
                "step": 4,
                "name": "RetrievalPolicy 分桶",
                "description": "当前默认 same_role_all_mbti 使用同角色 active 策略池；hybrid_role_mbti_global 和 hybrid_role_alignment_phase 保留为对比实验与兜底策略。",
                "code": "backend/agents/cognitive/retrieval_prod.py",
            },
            {
                "step": 5,
...
def render_report(evidence: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "默认策略 `same_role_all_mbti` 的单角色路径为：先按当前角色约束 active 策略池，再在桶内按关键词、阶段、动作类型、角色和质量进行重排。`hybrid_role_mbti_global` 仍可用于对比实验，但当前最高精度运行口径使用同角色策略池，避免低覆盖的 exact role+MBTI 池和泛 global 兜底把相关策略挤出 Top-3。",
        "",
        "### 1.1 单角色检索步骤",
        "",
        table(
        ...
        "",
        "Policy 对比显示，`same_role_all_mbti` 在当前离线 query set 中提供最高 Top-3 精度和稳定覆盖，`same_role_same_mbti` 因 MBTI 细分策略不足而频繁为空；hybrid 策略保留为消融和兜底对照，不作为当前最高精度默认口径。",
        "",
        "## 7. 单角色知识池规模",
        "",
        table(
