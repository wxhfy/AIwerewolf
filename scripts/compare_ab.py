...
def run_one(seed, disable=False):
    ...
    try:
        from scripts.llm_game_smoke import _run_one, _assert_strict_full_game
        t0 = time.time()
        ...
        return {
            "seed": seed, "winner": state.winner.value, "day": state.day,
            "decisions": len(state.decision_records),
            "calls": tracker["calls"], "input_tokens": tracker["input"],
            "output_tokens": tracker["output"],
            "total_tokens": tracker["input"] + tracker["output"],
            "time_s": round(elapsed, 1), "fallbacks": fb, "invalids": inv,
        }
    finally:
...
def main():
    ...
    for mode, disabled in [("baseline", True), ("optimized", False)]:
        ...
        for s in SEEDS:
            try:
                ...
                print(f"  seed={s} {r['winner']:<8} day={r['day']}  calls={r['calls']:>3}  tokens={r['total_tokens']:>8,}  {r['time_s']}s")
    ...
    for name, key, fmt, lb in [
        ...
        print(f"  {name:<35} {bv:>{len(fmt)+4},{fmt}} {ov:>{len(fmt)+4},{fmt}} {delta:>+10,.1f} {d}{abs(pct):>6.1f}%")
