...
class BGEEngine:
    ...
    def __init__(self, docs, model_path=BGE_PATH, device=GPU):
        ...
        print(f"  BGE Engine: {self.N} docs, dense={self.dense.shape}, "
    ...
    def search_dense(self, q, k=20):
        ...
        return [{"idx":int(i),"score":float(sims[i]),"situation":self.docs[i]["situation"],
                 "strategy":self.docs[i]["strategy"],"quality":self.docs[i]["quality"]} for i in idx]
    ...
    def search_sparse(self, q, k=20):
        ...
        idx = np.argsort(scores)[::-1][:k]
        return [{"idx":int(i),"score":float(scores[i]),"situation":self.docs[i]["situation"],
                 "strategy":self.docs[i]["strategy"],"quality":self.docs[i]["quality"]} for i in idx]
    ...
    def _role_bonus(self, q):
        return np.array([0.1 if d["role"]==q["role"] else (0.03 if d["role"]=="global" else 0) for d in self.docs], dtype=np.float32)

    def _phase_bonus(self, q):
        return np.array([0.05 if d["phase"]==q["phase"] else (0.02 if d["phase"]=="global" else 0) for d in self.docs], dtype=np.float32)
...
def rrf_fuse(list_a, list_b, docs, k=20, rrf_k=RRF_K):
    ...
    return [{"idx":i,"rrf_score":s,"situation":docs[i]["situation"],
             "strategy":docs[i]["strategy"],"quality":docs[i]["quality"]} for i, s in top]
...
def dynamic_alpha_fuse(list_a, list_b, docs, entropy, k=20):
    ...
    idx = np.argsort(scores)[::-1][:k]
    return [{"idx":int(i),"situation":docs[i]["situation"],
             "strategy":docs[i]["strategy"],"quality":docs[i]["quality"]} for i in idx]
...
def evaluate(results, query):
    ...
    for rank, r in enumerate(results, 1):
        if relevance(r, query) > 0: m["MRR"] = 1.0/rank; break
    else: m["MRR"] = 0.0
    for k in [5, 10]:
        kk = min(k, len(results))
...
def main():
    ...
    methods["D_BGE_DS_RRF"] = ("BGE DS-RRF",
        lambda q: rrf_fuse(bge.search_dense(q, k=20), bge.search_sparse(q, k=20), docs, k=10))

    # E: BM25 + BGE Dense RRF (FIXED)
    methods["E_BM25+BGE_RRF"] = ("BM25+BGE RRF",
        lambda q: rrf_fuse(bm25.search(q, k=20), bge.search_dense(q, k=20), docs, k=10))

    # F: BGE DS-RRF + ColBERT rerank
    methods["F_DS_RRF+ColBERT"] = ("DS-RRF+ColBERT",
        lambda q: bge.colbert_rerank(q,
            rrf_fuse(bge.search_dense(q, k=20), bge.search_sparse(q, k=20), docs, k=20), k=10))
    ...
    def full_pipeline(q):
        ...
        candidates = rrf_fuse(bge.search_dense(q_expanded, k=20),
                              bge.search_sparse(q_expanded, k=20), docs, k=20)
    ...
    for mname, (mlabel, mfunc) in methods.items():
        ...
        for q in queries:
            ...
            lats.append(lat)
            m = evaluate(res, q); m["latency_ms"] = lat; metrics_list.append(m)
        ...
        print(f"\n{mname} ({mlabel}):")
        print(f"  ALL:  P@5={agg['P@5']:.3f} P@10={agg['P@10']:.3f} MRR={agg['MRR']:.3f} "
    ...
    ranked = sorted(methods.keys(),
                    key=lambda m: np.mean([q["NDCG@5"] for q in results[m]["all"]]), reverse=True)

    for rank, mname in enumerate(ranked, 1):
        label = methods[mname][0]
        agg = {k: np.mean([q[k] for q in results[mname]["all"]]) for k in ["P@5","P@10","MRR","NDCG@5","NDCG@10","latency_ms"]}
        ...
        for mk in ["P@5","P@10","MRR","NDCG@5","NDCG@10"]:
            if mname == best(mk): markers.append(mk)
        star = " ⭐" if len(markers) >= 2 else ""

        print(f"{rank:<3} {label:<24} {agg['P@5']:>7.3f} {agg['P@10']:>7.3f} {agg['MRR']:>7.3f} "
    ...
    quality_candidates = {mn: np.mean([q["NDCG@5"] for q in results[mn]["all"]])
                         for mn in methods if np.mean([q["latency_ms"] for q in results[mn]["all"]]) < 200}
    best_q = max(quality_candidates, key=quality_candidates.get)

    # Find fastest with NDCG > 0.75
    fast_candidates = {mn: np.mean([q["latency_ms"] for q in results[mn]["all"]])
                      for mn in methods if np.mean([q["NDCG@5"] for q in results[mn]["all"]]) > 0.65}
