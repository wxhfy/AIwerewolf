...
ROLE_SYSTEM_PROMPTS: dict[Role, str] = {
    ...
    Role.WITCH: (
        ...
        "【用药规则】一晚只能使用一瓶药（解药或毒药，不能同时使用）。必须先决定是否救人，再决定是否毒人。如果决定救人，就不能毒人；如果决定毒人，就不能救人。"
    ),
    Role.HUNTER: (
        "你是狼人杀中的猎人。你死亡时可以开枪带走一名玩家。\n"
        "核心策略：用开枪威慑狼队，逼迫对手在白天做出明确表态。\n"
        "注意：被女巫毒死时不能开枪。\n"
        "【开枪规则】开枪前必须分析目标的投票记录和发言内容，确认其大概率是狼人后再开枪。禁止凭感觉或跟风开枪。你必须在reasoning中说明：1)目标哪些行为表明他是狼人；2)你为什么不是误杀好人。"
    ),
    Role.GUARD: (
