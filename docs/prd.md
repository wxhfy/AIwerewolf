...
### 1.1 角色系统（首发 6 角色，架构预留扩展）

| 角色 | 阵营 | 能力 | 使用限制 |
|------|------|------|----------|
| 村民 | 好人 | 无特殊能力，靠发言推理投票 | — |
| 狼人 | 狼人 | 夜晚击杀一人；内部可交流协商；**允许自刀** | 每晚一次 |
| 预言家 | 好人 | 夜晚查验一人身份（好人/狼人）；**被刀当夜查验仍有效** | 每晚一次 |
| 女巫 | 好人 | 拥有一瓶解药和一瓶毒药；两种药**可同夜使用**，但**不可对同一目标同时使用** | 解药仅首夜可自救；其余夜不可自救 |
...
NIGHT 子阶段（串行，但所有行动视为同时发生）：
  GUARD_CHOOSE → WOLF_DISCUSS → WOLF_KILL → SEER_CHECK → WITCH_ACTION
  → 夜晚结算（确定实际死亡者）→ HUNTER_SHOOT_NIGHT(猎人实际死于狼刀时触发)
  → CHECK_WIN(仅在所有夜晚行动结算完成后判定)

DAY 子阶段：
  DEATH_ANNOUNCE(含遗言，多死按座位号顺序公布) → SPEECH(3轮) → VOTE
  → VOTE_TIE?(PK发言 + PK投票) → ELIMINATE
  → HUNTER_SHOOT_DAY(猎人被投票出局时触发)
  → CHECK_WIN(投票淘汰或猎人开枪后判定)

GAME_OVER 子阶段：
  REVEAL_ALL → STATS → RETURN_TO_ROOM

规则：夜晚所有行动视为同时发生。角色在夜晚死亡不影响其当晚行动的有效性。
     例：被狼刀的预言家仍可获得查验结果；被刀女巫仍可使用药品。

CHECK_WIN 触发时机（三处）：
  - 夜晚结算完成后（所有行动 + 猎人夜间开枪后，一次性判定）
  - 投票淘汰后
  - 猎人白天开枪后
胜负一旦触发，立即中断当前阶段，进入 GAME_OVER。
```
...
- 3 轮发言 = 每位存活玩家每轮发言 1 次，共 3 轮
- 默认发言顺序：从上一轮被淘汰者的下一位开始（按座位号逆时针），每轮不变
- 首轮：从随机玩家开始
- 发言阶段被淘汰的玩家不参与本轮后续发言
