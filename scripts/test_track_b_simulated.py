...
def build_simulated_game():
    ...
    def ev(day, phase, etype, payload, vis="public"):
        state.events.append(
            GameEvent(
                ...
                type=etype,
                payload=payload,
                ts=float(day * 1000 + len(state.events)),
                visibility=vis,
            )
        )
    ...
    ev(
        ...
        {"actor_id": "P1", "actor_name": "宋知野", "speech": "我竞选警长。我是预言家，查3号于川金水。"},
    )
    ev(
        1,
        Phase.DAY_BADGE_SPEECH,
        EventType.CHAT_MESSAGE,
        {"actor_id": "P3", "actor_name": "于川", "speech": "我是真预言家！验4号陈小玉是狼人！"},
    )
    for vid, vname in [("P1", "宋知野"), ("P2", "白晓晴"), ("P7", "夏小满")]:
        ev(
            ...
            EventType.VOTE_CAST,
            {"voter_id": vid, "target_id": "P1", "voter_name": vname, "target_name": "宋知野"},
        )
    for vid, vname in [("P3", "于川"), ("P4", "陈小玉"), ("P5", "雷昊"), ("P6", "卓砚")]:
        ev(
            ...
            Phase.DAY_BADGE_ELECTION,
            EventType.VOTE_CAST,
            {"voter_id": vid, "target_id": "P3", "voter_name": vname, "target_name": "于川"},
        )
    ...
    ev(
        ...
        Phase.DAY_SPEECH,
        EventType.CHAT_MESSAGE,
        {"actor_id": "P3", "actor_name": "于川", "speech": "查验4号陈小玉是狼人，今天归票4号。"},
    )
    ev(
        1,
        Phase.DAY_SPEECH,
        EventType.CHAT_MESSAGE,
        {"actor_id": "P1", "actor_name": "宋知野", "speech": "3号悍跳！我才是真预言家。今天出4号也行。"},
    )
    ev(
        1,
        Phase.DAY_SPEECH,
        EventType.CHAT_MESSAGE,
        {"actor_id": "P4", "actor_name": "陈小玉", "speech": "我是女巫！昨晚自救。3号于川是悍跳狼！"},
    )
    ev(
        1,
        Phase.DAY_SPEECH,
        EventType.CHAT_MESSAGE,
        {"actor_id": "P7", "actor_name": "夏小满", "speech": "跟警长走，出4号。4号跳女巫但没报银水不可信。"},
    )
    ...
    for v, t in [("P1", "P4"), ("P2", "P4"), ("P3", "P4"), ("P5", "P4"), ("P7", "P4")]:
        ev(
            ...
            EventType.VOTE_CAST,
            {
                "voter_id": v,
                "target_id": t,
                "voter_name": next(p.name for p in players if p.id == v),
                "target_name": "陈小玉",
            },
        )
    ...
    ev(
        ...
        {"player_id": "P4", "player_name": "陈小玉", "reason": "vote", "role": "Witch"},
    )
    ev(
        1,
        Phase.DAY_LAST_WORDS,
        EventType.CHAT_MESSAGE,
        {"actor_id": "P4", "actor_name": "陈小玉", "speech": "遗言：我是女巫。3号于川是狼。被票出去是好人巨大失误。"},
    )
    ...
    ev(
        ...
        Phase.DAY_SPEECH,
        EventType.CHAT_MESSAGE,
        {"actor_id": "P1", "actor_name": "宋知野", "speech": "于川死了他是狼！我查5号雷昊金水。出7号夏小满。"},
    )
    ev(
        2,
        Phase.DAY_SPEECH,
        EventType.CHAT_MESSAGE,
        {"actor_id": "P7", "actor_name": "夏小满", "speech": "我是村民！4号遗言说3号是狼，1号也可能是狼！"},
    )
    ev(
        2,
        Phase.DAY_SPEECH,
        EventType.CHAT_MESSAGE,
        {"actor_id": "P6", "actor_name": "卓砚", "speech": "7号说得有道理。1号的查验很可疑。"},
    )
    ...
    ev(
        ...
        Phase.DAY_VOTE,
        EventType.VOTE_CAST,
        {"voter_id": "P6", "target_id": "P1", "voter_name": "卓砚", "target_name": "宋知野"},
    )
    ...
    ev(
        ...
        Phase.DAY_SPEECH,
        EventType.CHAT_MESSAGE,
        {"actor_id": "P1", "actor_name": "宋知野", "speech": "三人局我是预言家5号金水。出7号。"},
    )
    ev(
        3,
        Phase.DAY_SPEECH,
        EventType.CHAT_MESSAGE,
        {"actor_id": "P5", "actor_name": "雷昊", "speech": "我是猎人。信1号，出7号。"},
    )
    ev(
        3,
        Phase.DAY_VOTE,
        EventType.VOTE_CAST,
        {"voter_id": "P1", "target_id": "P7", "voter_name": "宋知野", "target_name": "夏小满"},
    )
    ev(
        3,
        Phase.DAY_VOTE,
        EventType.VOTE_CAST,
        {"voter_id": "P5", "target_id": "P7", "voter_name": "雷昊", "target_name": "夏小满"},
    )
    ev(
        3,
        Phase.DAY_VOTE,
        EventType.VOTE_CAST,
        {"voter_id": "P7", "target_id": "P1", "voter_name": "夏小满", "target_name": "宋知野"},
    )
    ...
    ev(
        ...
        {"hunter_id": "P5", "hunter_name": "雷昊", "target_id": "P1", "target_name": "宋知野", "reasoning": "带狼走！"},
    )
...
def main():
    ...
    from backend.eval.track_b import PublishedReviewDocument
    from backend.eval.track_b import VisualReportAgent
    ...
    for s in sorted(metrics.player_scores, key=lambda s: -s.final_score):
        ...
        print(
            f"  {s.player_name:6s}({s.role:>10s}): {s.final_score:5.1f} {bar} vote={s.vote_score:.2f} speech={s.speech_score:.2f} skill={s.skill_score:.2f}"
        )
    ...
    for bc in bad_cases:
        ...
        if bc.suggested_fix:
            print(f"    Fix: {bc.suggested_fix[:110]}")
    ...
    doc = PublishedReviewDocument(
        ...
        replay_bundle={
            ...
            "votes": [
                {
                    "day": e.day,
                    "voter_id": e.payload.get("voter_id"),
                    "target_id": e.payload.get("target_id"),
                    "voter_name": e.payload.get("voter_name", ""),
                    "target_name": e.payload.get("target_name", ""),
                }
                for e in state.events
                if e.type.value == "VOTE_CAST"
            ],
            "deaths": [],
    ...
    svg_methods = [
        ("Banner", agent.render_story_banner),
        ("Timeline", agent.render_timeline_ribbon),
