...
def evaluate(results, q):
    ...
    for rank, r in enumerate(results, 1):
        ...
        if relevance(idx, q) > 0: m["MRR"] = 1.0/rank; break
    else: m["MRR"] = 0.0
    for k in [5, 10]:
        kk = min(k, len(results))
...
class AgentGrep:
    ...
    def search_with_synonyms(self, q, topk=10):
        ...
        synonym_map = {
            "解药": ["救人", "救药"], "毒药": ["毒人", "撒毒"],
            "银水": ["被救", "救过"], "查杀": ["查验狼人", "验出狼"],
            "金水": ["查验好人", "验出好人"], "悍跳": ["假跳", "冒充"],
            "自爆": ["自爆身份"], "冲票": ["统一投票", "绑票"],
            "表水": ["自证", "清白"], "归票": ["带票", "带队投票"],
            "警徽流": ["警徽", "验人顺序"], "对跳": ["对跳", "假跳"],
        }
        query_words = [w for w in jieba.cut(q["text"]) if len(w) >= 2]
        ...
        for i, d in enumerate(self.docs):
            if d["role"] == q["role"]: scores[i] += 1.0
            if d["phase"] == q["phase"]: scores[i] += 0.5
...
class BM25Search:
    ...
    def search(self, q, topk=10):
        ...
        idx = np.argsort(scores)[::-1][:topk]
        return [{"idx":int(i),"situation":self.docs[i]["situation"],
...
if __name__ == "__main__":
    ...
    for mname, mfunc in methods.items():
        ...
        base_m = metrics_list[:20]; spec_m = metrics_list[20:]
    ...
    ref_metrics = {"ref_bm25_rrf": {"NDCG@5": 0.723, "P@5": 0.394, "MRR": 0.734, "Base": 0.874, "Spec": 0.522, "Lat": 70},
                   "ref_grep_basic": None, "ref_grep_syn": None, "ref_bm25": None}
    ...
    for mname, res in all_results.items():
        ...
        print(f"{mname:<30} {agg['NDCG@5']:>8.3f} {agg['P@5']:>8.3f} {agg['MRR']:>8.3f} {base_n:>8.3f} {spec_n:>8.3f} {lat:>7.1f}ms")

    # Reference: our production system results
    print(f"{'BM25 + BGE Dense RRF (production)':<30} {'0.723':>8} {'0.394':>8} {'0.734':>8} {'0.874':>8} {'0.522':>8} {'70':>7}ms")
