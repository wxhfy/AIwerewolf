...
def bootstrap_ci(data: list[float], n_bootstrap: int = 10000, ci: float = 0.95) -> dict:
    ...
    arr = np.array(data)
    means = [float(np.mean(np.random.RandomState(42).choice(arr, len(arr), replace=True)))
...
def run_one_game(tier: str, seed: int, env_vars: dict, player_count: int = 7) -> dict:
    ...
    try:
        ...
        for p in game.state.players:
            ...
            won = ((winner == "wolf" and is_wolf) or
                   (winner == "village" and not is_wolf))
            mbti = "UNKNOWN"
            ...
            players.append({"name": p.name, "role": role, "mbti": mbti,
                            "team": "wolf" if is_wolf else "village", "won": won})
        return {"tier": tier, "seed": seed, "winner": winner,
                "day": game.state.day, "duration_s": dur, "players": players}
    except Exception as e:
        import traceback
        return {"tier": tier, "seed": seed, "error": str(e)[:300],
                "traceback": traceback.format_exc()[-500:],
                "duration_s": int(time.time() - t0), "players": []}
...
def generate_report(all_results: list[dict], tier_names: list[str]) -> str:
    ...
    L = []; S = "=" * 90
    L.append(S); L.append("MULTI-TIER EXPERIMENT REPORT")
    L.append(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    L.append(f"Provider: {os.getenv('LLM_PROVIDER','?')} | Model: {os.getenv('DSV4FLASH_MODEL','?')}"); L.append(S)
    ...
    for t in tier_names:
        gs = tier_games.get(t, [])
        n = len(gs)
        if n == 0: L.append(f"{t:<16} {0:>6} {'--':>12} {'--':>12} {'--':>9}"); continue
    ...
    for team_name, team_label in [("village", "Village (好人阵营)"), ("wolf", "Wolf (狼人阵营)")]:
        ...
        L.append("-" * 80); base_wr = None
        for t in tier_names:
            gs = tier_games.get(t, [])
            outcomes = [1.0 if p["won"] else 0.0 for g in gs for p in g.get("players",[]) if p["team"]==team_name]
            if not outcomes: L.append(f"{t:<16} {0:>6} {'--':>12}"); continue
            ci = bootstrap_ci(outcomes)
            wr = ci["mean"]
            if base_wr is None: base_wr = wr; ds = "--"
            else: ds = f"{(wr-base_wr)*100:+.1f}%"
            L.append(f"{t:<16} {len(gs):>6} {wr*100:>10.1f}%  [{ci['ci_lower']*100:.1f}–{ci['ci_upper']*100:.1f}%]  {ds:>12}")
    ...
    for role in ["Seer","Witch","Hunter","Guard","Villager","Werewolf"]:
        ...
        for t in tier_names:
            ...
            if not outcomes: L.append(f"{t:<16} {0:>8} {'--':>12}"); continue
            ci = bootstrap_ci(outcomes)
            wr = ci["mean"]
            if base_wr is None: base_wr = wr; ds = "--"
            else: ds = f"{(wr-base_wr)*100:+.1f}%"
            L.append(f"{t:<16} {len(outcomes):>8} {wr*100:>10.1f}%  [{ci['ci_lower']*100:.1f}–{ci['ci_upper']*100:.1f}%]  {ds:>12}")

    # ── 4. MBTI win rates ──
    L.append(f"\n{'─'*90}\n## 4. MBTI WIN RATES\n")
    all_mbti = sorted({p.get("mbti","UNKNOWN") for t in tier_names for g in tier_games.get(t,[]) for p in g.get("players",[]) if p.get("mbti") and p["mbti"]!="UNKNOWN"})
    for mbti in all_mbti:
        L.append(f"\n### {mbti}\n")
        L.append(f"{'Tier':<16} {'Samples':>8} {'WinRate':>12} {'vs Baseline':>16}")
        L.append("-" * 60); base_wr = None
        for t in tier_names:
            gs = tier_games.get(t, [])
    ...
    for t in tier_names:
        gs = tier_games.get(t, [])
        n = len(gs); errs = sum(1 for r in all_results if r["tier"]==t and "error" in r)
...
def main():
    ...
    if not tiers: print(f"ERROR: choose from {list(ALL_TIERS)}"); sys.exit(1)
    ...
    if res_file.exists():
        with open(res_file) as f:
            for line in f:
                try: r = json.loads(line.strip()); done_set.add((r["tier"],r["seed"]))
                except: pass
    ...
    pending = len(queue); done = total - pending
    ...
    if pending == 0:
        ...
        print(rep); rep_file.write_text(rep, encoding="utf-8")
        print(f"\nReport: {rep_file}"); return
    ...
    for idx, (tier, seed, env) in enumerate(queue):
        ...
        if "error" in result:
            ...
            print(f"  DONE ({d}s/{d/60:.1f}min): Winner={result.get('winner','?')}, Day={result.get('day','?')}", flush=True)
    ...
    all_r = [json.loads(l) for l in res_file.read_text().splitlines() if l.strip()]
    rep = generate_report(all_r, tier_names)
    rep_file.write_text(rep, encoding="utf-8"); print(rep)
    print(f"\nResults: {res_file}\nReport:  {rep_file}")
