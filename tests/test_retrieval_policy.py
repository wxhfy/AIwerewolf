...
def test_keyword_search_returns_role_phase_and_action_scope_for_evaluation() -> None:
    results = _retriever().search_with_keywords(
        ["查杀"],
        role="Seer",
        phase="DAY_SPEECH",
    ...
    assert results[0]["role"] == "Seer"
    assert results[0]["phase"] == "DAY_SPEECH"
    assert results[0]["role_scope"] == "Seer"
    assert results[0]["phase_scope"] == "DAY_SPEECH"
    assert results[0]["action_scope"] == "check"


def test_keyword_search_requires_lexical_hit_before_context_bonus() -> None:
    retriever = StrategyRetriever()
    assert (
        retriever.build_from_docs(
            [
                {
                    "doc_id": "generic-high-quality",
                    "situation": "当你拿到狼人牌，出现重复决策模式时",
                    "strategy": "狼人白天推进关键神职出局时，要保持发言逻辑和队友配合一致。",
                    "role": "Werewolf",
                    "phase": "DAY_SPEECH",
                    "quality": 0.99,
                },
                {
                    "doc_id": "wolf-check-defense",
                    "situation": "被真预言家查杀怎么应对",
                    "strategy": "被查杀时先表水，再选择反跳预言家或解释验人逻辑漏洞。",
                    "role": "Werewolf",
                    "phase": "DAY_SPEECH",
                    "quality": 0.85,
                },
    ...
    results = retriever.search_with_keywords(
        ...
        k=2,
        retrieval_policy=RetrievalPolicy.SAME_ROLE_ALL_MBTI,
        agent_context=AgentContext(role="Werewolf", phase="DAY_SPEECH", mbti="INTJ", alignment="wolf"),
    )

    assert [item["doc_id"] for item in results] == ["wolf-check-defense"]
    assert results[0]["keyword_overlap_score"] > 0
...
def test_agent_factory_defaults_to_same_role_retrieval_policy(monkeypatch) -> None:
