...
def _pair_degenerate_rate(pairs, reg=None):
    ...
    for p in pairs:
        ...
        td = sum(abs(float(bf.get(k, 0)) - float(gf.get(k, 0)))
                 for k in all_k if isinstance(bf.get(k), (int, float)) and isinstance(gf.get(k), (int, float)))
        if td < 1e-6: deg += 1
...
class TestPairwiseExpansion:
    ...
    def test_no_player_id_shortcut_in_expansion_pairs(self):
        for path in ["pairwise_vote_expansion_examples.jsonl",
                     "pairwise_night_action_expansion_examples.jsonl"]:
    ...
    def test_per_action_rankers_train_and_predict(self):
        from backend.eval.pairwise_ranker import (
            PairwiseLogisticRanker, PerActionPairwiseRankers,
        )
        ...
        for ptype, base_features, n in [
            ...
            for i in range(n):
                ...
                all_pairs.append(PairwiseExample(
                    pair_id=f"{ptype}-{i:04d}", source="test", role="Werewolf",
                    action_type=ptype, better_features=bf, worse_features=wf,
                ))
    ...
    def test_effective_pair_metrics_computable(self):
        ...
        all_pairs = []
        for f in ["pairwise_vote_expansion_examples.jsonl",
                  "pairwise_night_action_expansion_examples.jsonl"]:
        ...
        for ptype, ps in sorted(by_type.items()):
            ...
            print(f"  {ptype:30s}: n={len(ps):3d} effective={len(ps)-deg} "
...
def test_hard_cap_count_is_zero():
    ...
    from backend.eval.scoring_models import calibrate_decision_quality, extract_features
