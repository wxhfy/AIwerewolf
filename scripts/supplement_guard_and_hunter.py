...
def create_guard_golden_cases(opps: list[dict]) -> list[dict]:
    ...
    for opp in protect_opps:
        ...
        if is_key_target and target_is_exposed:
            golden.append({
                ...
                "role": "Guard", "opportunity_type": "guard_protect",
                "task_type": "single_action", "day": opp["day"],
                "label": {
                    "quality_score": 92,
                    "role_alignment": 90,
                    "risk_level": "low",
        ...
        if actor_died:
            golden.append({
                "opportunity_id": opp["opportunity_id"],
                "role": "Guard", "opportunity_type": "guard_protect",
                "task_type": "mistake_severity", "day": opp["day"],
                "label": {
                    "severity_score": 70,
                    "is_critical": True,
                    "recoverable": False,
    ...
    for opp in vote_opps:
        ...
        if is_wolf:
            golden.append({
                "opportunity_id": opp["opportunity_id"],
                "role": "Guard", "opportunity_type": "vote",
                "task_type": "single_action", "day": day,
                "label": {
                    "quality_score": 82,
                    "role_alignment": 80,
                    "risk_level": "low",
    ...
    for opp in speech_opps[:15]:
        golden.append({
            "opportunity_id": opp["opportunity_id"],
            "role": "Guard", "opportunity_type": "speech",
            "task_type": "speech_quality", "day": opp.get("day", 1),
            "label": {
                "groundedness": 15,
                "stance_clarity": 12,
                "consistency": 14,
...
def create_hunter_optimized_cases(opps: list[dict]) -> dict:
    ...
    error_analysis = {
        ...
        "shot_wolf": 0, "shot_good": 0,
        "good_restraint": 0, "missed_shot": 0,
        "no_evidence_random_shot": 0, "high_suspicion_no_shot": 0,
    }
    ...
    for opp in shot_opps:
        ...
        if is_wolf and target_died:
            ...
            golden.append({
                "opportunity_id": opp["opportunity_id"],
                "role": "Hunter", "opportunity_type": "hunter_shot",
                "task_type": "single_action", "day": opp["day"],
                "label": {
                    "quality_score": 95,
                    "role_alignment": 95,
                ...
                "golden_case": True, "case_type": "shot_wolf",
            })
        # Shot good → hunter_shot_quality LOW
            ...
            golden.append({
                ...
                "golden_case": True, "case_type": "shot_good",
            })
        # No clear evidence → random shot
            ...
            golden.append({
                ...
                "golden_case": True, "case_type": "no_evidence_shot",
            })
    ...
    for opp in vote_opps[:20]:
        ...
        golden.append({
            "opportunity_id": opp["opportunity_id"],
            "role": "Hunter", "opportunity_type": "vote",
            "task_type": "single_action", "day": opp["day"],
            "label": {
                ...
                "risk_level": "low",
                "confidence": 0.80,
                "reason": f"OPTIMIZED: 猎人投票{'命中狼人' if is_wolf else '误投好人'}。hunter_restraint_quality=GOOD" if is_wolf else "NEUTRAL。",
    ...
    for opp in speech_opps[:10]:
        golden.append({
            "opportunity_id": opp["opportunity_id"],
            "role": "Hunter", "opportunity_type": "speech",
            "task_type": "speech_quality", "day": opp["day"],
            "label": {
                "groundedness": 16,
                "stance_clarity": 13,
                "consistency": 14,
            ...
            "golden_case": True, "case_type": "speech_hidden",
        })
...
def add_guard_features(opps: list[dict]) -> int:
    ...
    for opp in opps:
        ...
        is_repeat_guard = (previous_guard_target is not None and
                          chosen.get("target") == previous_guard_target) if isinstance(chosen, dict) else False
...
def main() -> int:
    ...
    for opp in guard_golden:
        ...
        if not target.get("actual_block") and target.get("target_alignment") == "village" and target.get("is_key_role_exposed"):
    ...
    with open(ROOT / "data/health/guard_error_analysis.md", "w") as f:
        ...
        f.write(f"## False Positives (lucky outcomes scored too high)\n")
    ...
    print(f"Hunter error analysis → data/health/hunter_error_analysis.md")
