...
import torch; from torch.utils.data import DataLoader, Dataset; import torch.nn as nn
...
def llm_verify_batch(pairs):
    ...
    try:
        r = requests.post(LU, headers={"Authorization":f"Bearer {LK}","Content-Type":"application/json"},
            json={"model":"deepseek-v4-flash","messages":[{"role":"user","content":p}],"temperature":0.1,"max_tokens":300}, timeout=30)
        c = r.json()["choices"][0]["message"]["content"]
        m = re.search(r'\[.*\]', c, re.DOTALL)
        return json.loads(m.group()) if m else [{"id":i+1,"is_true_negative":True,"conf":0.5} for i in range(len(pairs))]
    except: return [{"id":i+1,"is_true_negative":True,"conf":0.5} for i in range(len(pairs))]

class TD(Dataset):
    def __init__(s, t, tok, ml=256): s.t=t; s.tok=tok; s.ml=ml
...
def evaluate(model_path, docs, queries):
    ...
    m = SentenceTransformer(model_path, device=GPU); N = len(docs)
    ...
    for q in queries:
        ...
        ds = np.dot(de,qe)+np.array([0.12 if d["role"]==q["role"] else (0.03 if d["role"]=="global" else 0) for d in docs],dtype=np.float32)
        ...
        for k in [5,10]:
            kk=min(k,len(res))
            mv[f"P@{k}"]=sum(1 for r in res[:kk] if sum(1 for kw in q["kw"] if kw.lower() in f"{r['situation']} {r['strategy']}".lower())>0)/kk
        for rnk,r in enumerate(res,1):
            if sum(1 for kw in q["kw"] if kw.lower() in f"{r['situation']} {r['strategy']}".lower())>0: mv["MRR"]=1.0/rnk; break
        else: mv["MRR"]=0.0
        for k in [5,10]:
            kk=min(k,len(res))
            gs=[sum(1 for kw in q["kw"] if kw.lower() in f"{r['situation']} {r['strategy']}".lower()) for r in res[:kk]]
...
if __name__ == "__main__":
    ...
    bm = BM25Okapi([t.split() for t in c2])
    ri={}; [ri.setdefault(d["role"],[]).append(i) for i,d in enumerate(docs)]
    ...
    for i, anchor in enumerate(docs):
        ...
        mask = np.ones(len(docs), bool); mask[i]=False
        for j in same: mask[j]=False
    ...
    from sentence_transformers import SentenceTransformer; from transformers import AutoTokenizer
    device = torch.device(GPU)
    tokenizer = AutoTokenizer.from_pretrained(BGP)
    model = SentenceTransformer(BGP, device=GPU); model.train()
    ...
    for step, (enc, n) in enumerate(dl):
        ...
        ps=(a*p).sum(dim=1)/0.05; ns=(a*nn).sum(dim=1)/0.05
        loss = -ps.mean()+torch.logsumexp(torch.stack([ps,ns],dim=1),dim=1).mean()
        opt.zero_grad(); loss.backward(); opt.step()
        if step%5==0: print(f"  Step {step}/{len(dl)}: Loss={loss.item():.4f}")

    # Save
    os.makedirs(OUT, exist_ok=True); import shutil
    ...
    for sub in ['1_Pooling']:
        ...
        if os.path.exists(sd) and not os.path.exists(dd): shutil.copytree(sd,dd)
    ...
    if a_ndcg > b_ndcg: print("✅ LLM verification improved fine-tuning!")
    else: print("⚠️ Still no improvement — zero-shot is the baseline")
