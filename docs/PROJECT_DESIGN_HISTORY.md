...
| Track B | 对每步决策做赛后复盘 | PerStepScorer、DecisionScore、ScoredStep、PublishedReview | 胜负无法解释行为质量 | `backend/eval/per_step_scorer.py`、`backend/eval/track_b.py` |
...
阶段结果是：对局引擎成为规则主控，Agent 不再直接操纵状态。后续所有信息隔离、审计、复盘和知识回流都建立在引擎事件和决策记录之上。
...
阶段结果是：每条 Agent 决策都能和它当时可见的事实边界对应起来。这样既保护了对局公平性，也为 Track B 复盘提供上下文。
...
复盘链路采用三级级联：确定性规则优先覆盖明确场景，Light LLM 处理模糊决策，Heavy LLM review panel 处理高影响或争议较大的决策。需要注意：85%/12%/3% 是设计说明和代码注释口径，正式报告若要写成实际比例，需要重新统计当前实验输出。
...
项目从“让 AI 能跑一局狼人杀”开始，逐步补齐信息隔离、决策审计、角色化认知、工具调用、策略检索、赛后复盘和知识回流。每一步演进都对应明确问题：流程混乱需要引擎主控，信息泄露需要 PlayerView，行为不可解释需要 AgentDecision 和 Track B，复盘无法复用需要 Track C，分散测试无法证明闭环则需要 strict mode。

最终系统已经形成产品级主线：Play 阶段完成对局，Review 阶段解释行为，Evolve 阶段沉淀知识。当前仍需补充的是更严格的多局平衡实验、Track B 复盘有效性验证、Track C 晋级后效果验证和前端展示验收。
