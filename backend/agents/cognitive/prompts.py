...
def build_think_prompt(
    ...
    strategy bias + role-specific anti-patterns from Track C bad case analysis.
    """
    game_ctx = build_game_context(obs)
    ...
    # Role-specific anti-patterns (closes Track C feedback loop)
...
# Maps each role × action to common mistakes that Track C's MetricsCalculator
# consistently detects. Injected into the agent's task description so agents
# actively avoid known failure patterns before making decisions.
...
def get_role_anti_patterns(role: str, action: str = "speech") -> str:
    ...
    These are derived from BadCase types that MetricsCalculator consistently
    detects (vote mistakes, ability misuse, speech errors). Injecting them
    directly into the task description primes the agent to actively avoid
    known failure patterns — closing the loop between Track C's mistake
    detection and agent decision-making.
    """
    role_patterns = _ROLE_ANTI_PATTERNS.get(role, {})
    patterns = role_patterns.get(action, [])
    if not patterns:
        # Fall back to speech patterns for any action without specific patterns
