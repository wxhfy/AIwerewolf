...
def build_queries():
    base = [
        ...
        {"id":"N15","role":"Villager","phase":"DAY_VOTE","text":"作为平民不确定该投谁怎么正确投票","kw":["平民投票","投票"]},
    ...
    special = [
        ...
        {"id":"E06","role":"Seer","phase":"DAY_BADGE_SPEECH","text":"我是预言家有查杀但对跳也出现了警徽流怎么安排","kw":["对跳","警徽流","查杀"]},
        {"id":"E07","role":"Werewolf","phase":"DAY_SPEECH","text":"既要伪装又要带节奏还要保护队友怎么同时做好","kw":["伪装","带节奏","保护队友"]},
        ...
        {"id":"E13","role":"global","phase":"mid_game","text":"预言家被刀女巫没救猎人被票带走了狼现在剩1狼2民1神怎么投","kw":["残局","轮次"]},
        {"id":"E14","role":"Cupid","phase":"night_1","text":"丘比特第一晚应该连谁比较好","kw":["丘比特","情侣"]},
        {"id":"E15","role":"Werewolf","phase":"NIGHT_WOLF_ACTION","text":"大野狼额外刀人应该怎么选目标","kw":["大野狼","双刀"]},
        {"id":"E16","role":"global","phase":"global","text":"如何通过表情和动作判断是不是狼","kw":["面杀","抿人"]},
        {"id":"E17","role":"global","phase":"global","text":"复盘发现自己每局都犯同样的错误怎么改进","kw":["复盘","学习"]},
...
class BM25Ret:
    ...
    def search(self, q, role="", phase="", k=20):
        ...
        scores += np.array([0.1 if d["role"]==q["role"] else (0.03 if d["role"]=="global" else 0) for d in self.docs])
        scores += np.array([0.05 if d["phase"]==q["phase"] else (0.02 if d["phase"]=="global" else 0) for d in self.docs])
        top = np.argsort(scores)[::-1][:k]
        return [{"situation":self.docs[i]["situation"],"strategy":self.docs[i]["strategy"],"quality":self.docs[i]["quality"]} for i in top]
...
class BGEBase:
    ...
    def __init__(self, docs, model_path=BGE_PATH, device=GPU):
        ...
        print(f"  BGE pre-computed: dense={self.dense_embs.shape}, sparse={len(self.sparse_embs)}, "
    ...
    def search_dense(self, q, k=20):
        ...
        scores = sims + np.array([0.15 if d["role"]==q["role"] else (0.05 if d["role"]=="global" else 0) for d in self.docs], dtype=np.float32)
        scores += np.array([0.08 if d["phase"]==q["phase"] else (0.03 if d["phase"]=="global" else 0) for d in self.docs], dtype=np.float32)
        top = np.argsort(scores)[::-1][:k]
        return [{"situation":self.docs[i]["situation"],"strategy":self.docs[i]["strategy"],"quality":self.docs[i]["quality"],"idx":int(i)} for i in top]
    ...
    def search_colbert_rerank(self, q, candidates_k=20, final_k=10):
        ...
        # Add bonuses
        scores += np.array([0.1 if self.docs[idx]["role"]==q["role"] else (0.03 if self.docs[idx]["role"]=="global" else 0) for idx in cand_indices])
        scores += np.array([0.05 if self.docs[idx]["phase"]==q["phase"] else (0.02 if self.docs[idx]["phase"]=="global" else 0) for idx in cand_indices])

        top = np.argsort(scores)[::-1][:final_k]
        return [{"situation":self.docs[cand_indices[i]]["situation"],
                 "strategy":self.docs[cand_indices[i]]["strategy"],
                 "quality":self.docs[cand_indices[i]]["quality"]} for i in top]
...
class HybridFusion:
    ...
    def rrf(self, list_a, list_b, k=20, rrf_k=60):
        ...
        return [{"situation":self.docs[i]["situation"],"strategy":self.docs[i]["strategy"],"quality":self.docs[i]["quality"]} for i,_ in top]
    ...
    def weighted(self, list_a, list_b, docs, w_a=0.5, w_b=0.5, k=20):
        ...
        top = np.argsort(scores)[::-1][:k]
        return [{"situation":docs[i]["situation"],"strategy":docs[i]["strategy"],"quality":docs[i]["quality"]} for i in top]
...
def main():
    ...
    add_method("D_BGE_DenseSparse",
        lambda q: fusion.rrf(bge.search_dense(q, k=20), bge.search_sparse(q, k=20), k=10))
    add_method("E_BM25+BGE_RRF",
        lambda q: fusion.rrf(
            [{"idx":i,"situation":r["situation"],"strategy":r["strategy"],"quality":r["quality"]}
             for i,r in enumerate(bm25.search(q, k=20))],
            bge.search_dense(q, k=20), k=10))
    add_method("F_BGE_SparseDense_RRF",
        lambda q: fusion.rrf(bge.search_sparse(q, k=20), bge.search_dense(q, k=20), k=10))
    add_method("G_ColBERT_Rerank",
        lambda q: bge.search_colbert_rerank(q, candidates_k=20, final_k=10))
    ...
    for mname, mfunc in methods.items():
        ...
        for q in all_queries:
            ...
            lats.append(lat)
            m = evaluate(res, q); m["latency_ms"] = lat; metrics.append(m)
        ...
        print(f"\n{mname}:")
        print(f"  ALL:    P@5={agg_all['P@5']:.3f} P@10={agg_all['P@10']:.3f} MRR={agg_all['MRR']:.3f} "
    ...
    candidates_quality = {mn: np.mean([q["NDCG@5"] for q in results[mn]["all"]])
                         for mn in methods if np.mean([q["latency_ms"] for q in results[mn]["all"]]) < 50}
    best_quality = max(candidates_quality, key=candidates_quality.get)

    # Find the fastest method with NDCG@5 > 0.80
    candidates_fast = {mn: np.mean([q["latency_ms"] for q in results[mn]["all"]])
                      for mn in methods if np.mean([q["NDCG@5"] for q in results[mn]["all"]]) > 0.80}
    best_fast = min(candidates_fast, key=candidates_fast.get)

    # Best overall (NDCG@5 per ms of latency)
    efficiency = {mn: np.mean([q["NDCG@5"] for q in results[mn]["all"]]) / max(np.mean([q["latency_ms"] for q in results[mn]["all"]]), 0.1)
                  for mn in methods}
