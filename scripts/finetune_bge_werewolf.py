...
def finetune(data_path, model_path=BGE_PATH, output_path=OUTPUT_PATH, epochs=3, batch_size=16, lr=2e-5):
...
def evaluate_retrieval(model_path, docs, queries, label=""):
    ...
    for q in queries:
        ...
        for rank, i in enumerate(b_top, 1):
            rrf_scores[int(i)] = rrf_scores.get(int(i), 0) + 1 / (60 + rank)
    ...
    print(
        f"    P@5={rrf_agg['P@5']:.3f}  P@10={rrf_agg['P@10']:.3f}  MRR={rrf_agg['MRR']:.3f}  "
