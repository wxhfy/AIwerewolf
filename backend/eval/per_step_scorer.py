...
class PerStepScorer:
    ...
    def score_vote(self, decision: dict, state: dict) -> DecisionScore:
        ...
        t_align = _align(state, tid); t_role = _role(state, tid)
        day = decision.get("day", 0)
        correct = 0.95 if t_align == "wolf" else (0.15 if t_role in _KEY_VILLAGE else 0.35)
        overall = round(0.50*correct + 0.25*_reasoning(decision.get("raw_text","")) + 0.10*min(0.9,0.5+day*0.08) + 0.15*_vote_weight(decision)*0.6, 3)
    ...
    def score_talk(self, decision: dict, speech_acts: list[dict], state: dict) -> DecisionScore:
        pid = decision.get("player_id",""); day = decision.get("day",0)
        act = next((a for a in speech_acts if a.get("player_id")==pid and a.get("day")==day), None)
        if act is None:
            return DecisionScore(
                decision_id=decision.get("id",""), player_id=pid,
                player_name=decision.get("player_name",""), role=decision.get("player_role",""),
                day=day, phase=decision.get("phase","DAY_SPEECH"), action_type="talk",
                ...
                evidence=["Speech not analyzed."], scoring_tier="deterministic",
            )
        stance=act.get("stance","neutral"); risks=len(act.get("risk_flags",[]))*0.15
        ...
        overall=round(0.40*correct+0.35*_reasoning(decision.get("raw_text",""))+0.10*0.7+0.15*impact,3)
        return DecisionScore(
            decision_id=decision.get("id",""), player_id=pid,
            player_name=decision.get("player_name",""), role=decision.get("player_role",""),
            day=day, phase=decision.get("phase","DAY_SPEECH"), action_type="talk",
            correctness=correct, reasoning_quality=_reasoning(decision.get("raw_text","")),
            timeliness=0.7, impact=impact, overall_score=overall,
            needs_light_llm=(0.25 < correct < 0.75),
            evidence=_talk_evidence(act), scoring_tier="deterministic",
        )

    def score_night(self, decision: dict, state: dict) -> DecisionScore:
        at=decision.get("action_type",""); tid=decision.get("target_id","")
        t_role=_role(state,tid); t_align=_align(state,tid)
        correct,ev=_night_correct(at,t_role,t_align)
        day=decision.get("day",0); impact=_night_impact(t_role,t_align)
        overall=round(0.55*correct+0.20*_reasoning(decision.get("raw_text",""))+0.10*0.8+0.15*impact,3)
        return DecisionScore(
            decision_id=decision.get("id",""), player_id=decision.get("player_id",""),
            player_name=decision.get("player_name",""), role=decision.get("player_role",""),
            day=day, phase=decision.get("phase",""), action_type=at,
            correctness=correct, reasoning_quality=_reasoning(decision.get("raw_text","")),
            timeliness=0.8, impact=impact, overall_score=overall,
            needs_light_llm=(0.25 < correct < 0.75),
            evidence=ev, scoring_tier="deterministic",
        )
    ...
    def score_all(
        self, decisions: list[dict], state: dict, speech_acts: list[dict],
        *, light_llm: bool = False, heavy_llm: bool = False,
    ) -> list[DecisionScore]:
...
def _alt_vote(s,tid):
    ...
    for p in s.get("players",[]):
        if p.get("alignment")=="wolf" and p.get("alive",True): return f"Better: {p.get('name','?')}({p.get('role','?')})"
    return ""
