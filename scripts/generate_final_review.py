...
def main() -> int:
    ...
    from scripts.train_and_ablate import rule_opportunity_value, rule_decision_quality
    ...
    print(f"  → review_with_learned_scores.md")
    ...
    print(f"  → review_with_learned_scores.json")
...
def _generate_markdown(reviews) -> str:
    lines = [
        ...
        f"**Scoring system**: Opportunity-aware learned evaluation (Track B v2)",
        "",
        "## Scoring Components",
...
def _run_valid_agent(reviews) -> dict:
    ...
    for r in reviews:
        ...
        if r["process_score"] > 100 or r["process_score"] < 0:
            issues.append({"player": r["player_id"], "type": "score_range", "detail": f"process_score={r['process_score']}"})
            passed = False
        if r["final_score"] > 100 or r["final_score"] < 0:
            issues.append({"player": r["player_id"], "type": "score_range", "detail": f"final_score={r['final_score']}"})
            passed = False

        # Check opportunity count
        if r["n_opportunities"] < 3:
            issues.append({"player": r["player_id"], "type": "low_opportunities", "detail": f"n={r['n_opportunities']}"})
            passed = False

        # Check speech score reasonableness
        if r["speech_score"] > 90:
            issues.append({"player": r["player_id"], "type": "speech_ceiling", "detail": f"speech={r['speech_score']:.1f}"})

        # Check model confidence
        if r["model_confidence"] < 0.3:
            issues.append({"player": r["player_id"], "type": "low_confidence", "detail": f"conf={r['model_confidence']:.2f}"})
