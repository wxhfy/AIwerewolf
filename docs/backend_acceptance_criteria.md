...
>
> 口径说明（2026-06-08）：本文是 2026-06-05 strict run 的验收记录，不是当前代码 schema 的唯一事实来源。当前 ORM 映射为 20 张核心表；正式对局 AI 席位是 LLM-only，`agent_type=heuristic` 会被拒绝，strict 模式不允许静默 heuristic fallback。

| 模块 | Status | 验收条件 | 失败条件 | 对应命令 | 输出文件 |
|------|--------|---------|---------|---------|---------|
| **DB** | ✅ Verified | PostgreSQL 可连接；当前代码口径为 20 张核心 ORM 表（games/players/game_events/agent_decisions/game_snapshots/votes/evaluations/agent_versions/leaderboard_entries/review_reports/published_reviews/strategy_knowledge_docs/strategy_graph_links/role_strategy_cards/persona_role_adapters/strategy_patches/evolution_tournaments/knowledge_usage_feedback/evolution_rounds/personas），历史实验快照可包含 experiments/strategy_snapshots 等额外表；FK 约束完整 | 连接失败或核心表缺失 | `scripts/run_backend_full_strict.py` Phase 0 | `backend_e2e_report.json` |
...
| **LLM-only / strict** | ✅ Verified | 主模型可用时使用 LLM 决策；strict 模式要求 `fallback_count=0`、`invalid_count=0`；测试可用 `_TEST_ALLOW_FAKE_LLM=true LLM_PROVIDER=fake`，正式对局不允许 heuristic seat 或静默 heuristic fallback | LLM 失败被静默替换为启发式动作；`agent_type=heuristic` 进入正式对局；strict 样本含 fallback/invalid 仍被当作有效结论 | Game Health Report 中的 fallback/invalid 计数 | `game_health_report.json` |
...
| **Track B Review (PerStepScorer)** | ✅ Verified | Tier 1 对所有决策产出初始 correctness 信号（100% 覆盖率，27/27）；Tier 2 对 ambiguous 决策触发单路 LLM 复核（light_llm_score 正确归一化 /10.0）；Tier 3 对 high-impact 决策触发多路 LLM 复核（score_step_with_panel 按单步决策处理，不做整局粗粒度判断）；每个 ScoredStep 包含 is_mistake/is_highlight 标记 | 决策记录缺失；Tier 分级逻辑错误；is_mistake 标记与实际判断偏离 > 30% | `per_step_scorer.py` + `llm_judge.py` | `scored_steps.json` |
...
- **Critic round**: 已通过 `score_step_with_panel()` 实现 — 多路 LLM 分别复核单步决策 → trimmed mean 聚合 → 可选 Critic 在分歧 > 0.3 时介入（权重 25%）
- **LLM 复核输出解析**: 使用 `re.search(r'\{.*\}', raw)` + `json.loads`，对 markdown 代码块格式敏感
- **多路复核串行调用**: 同一 LLM 以不同 system prompt 调用三次，非真正并行异构
- **PerStepScore.judge_agreement**: 新增字段记录多路复核标准差，供稳定性分析
