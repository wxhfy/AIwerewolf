"""Deterministic local LLM-compatible client for offline smoke runs.

The fake provider is intentionally gated in ``backend.llm.create_client`` by
``_TEST_ALLOW_FAKE_LLM=true``. Keeping the implementation under ``backend/``
prevents clean GitHub clones from failing when tests or local demos explicitly
choose the fake provider.
"""
...
class FakeLLMClient:
    """OpenAI-compatible deterministic client used by offline checks."""
    ...
    def chat_sync(self, messages: list[dict], **kwargs: Any) -> dict[str, Any]:
        ...
        if "=== 复盘任务 ===" in text or '"what_worked"' in text:
            content = json.dumps(
                {
                    "what_worked": ["遵守当前阶段的合法目标集合，行动具备可审计理由。"],
                    "what_failed": ["发言和投票之间的承接还可以更具体。"],
    ...
    def _legal_names(text: str) -> list[str]:
        ...
        if not legal_matches:
            return []
        return FakeLLMClient._extract_numbered_names(legal_matches[-1])
    ...
    def _alive_names(text: str) -> list[str]:
        ...
        if not matches:
            return []
        return FakeLLMClient._extract_numbered_names(matches[-1])

    @staticmethod
    def _extract_numbered_names(line: str) -> list[str]:
        names: list[str] = []
        pattern = re.compile(r"@?\d+号[:：]\s*(.*?)(?=(?:[，,、;；]\s*)?@?\d+号[:：]|$)")
    ...
    def _strategy_target(text: str, legal_names: list[str]) -> str:
        ...
        if role == "Seer":
            ...
            if available:
                return available[0]
        elif role == "Hunter" and available:
            return available[0]
