...
def _make_vote(day: int, voter: Player, target: Player) -> GameEvent:
    return GameEvent.create(
        ...
        visibility="public",
        payload={"voter_id": voter.id, "voter_name": voter.name,
                 "target_id": target.id, "target_name": target.name},
    )


def _make_speech(day: int, actor: Player, speech: str,
                 *, phase: Phase = Phase.DAY_SPEECH) -> GameEvent:
    return GameEvent.create(
...
def _make_night_action(day: int, actor: Player, action_type: str, target: Player,
                       *, phase: Phase = Phase.NIGHT_WOLF_ACTION) -> GameEvent:
    return GameEvent.create(
...
def _make_seer_result(day: int, seer: Player, target: Player, *, is_wolf: bool) -> GameEvent:
    return GameEvent.create(
        ...
        visibility="private",
        payload={"kind": "seer_result", "target_id": target.id,
...
def _make_wolf_tally(day: int, target: Player, votes: dict[str, str]) -> GameEvent:
    return GameEvent.create(
        ...
        visibility="private",
        payload={"kind": "wolf_attack_tally", "target_id": target.id,
                 "target_name": target.name, "votes": votes},
        visible_to=list(votes.keys()),
    )


def _make_death(day: int, player: Player, reason: str,
                *, phase: Phase | None = None) -> GameEvent:
    if phase is None:
...
def _make_decision(game_id: str, player: Player, role: Role, day: int, phase: str,
                   request: str, parsed_action: dict[str, Any],
                   *, observation: dict[str, Any] | None = None,
                   legal_actions: list[str] | None = None) -> DecisionAudit:
    return DecisionAudit(
        id=f"dec-{player.id}-{day}-{phase}-{_uid()}",
        game_id=game_id, player_id=player.id, day=day, phase=phase,
        request=request, observation=observation or {},
        legal_actions=legal_actions or [],
        prompt_version="v1", raw_output=None,
        parsed_action=parsed_action, is_valid=True,
...
def build_badcase_002_fixture() -> GameState:
    ...
    d1_sp3 = _make_speech(1, p3,
        "我昨晚查验 P1，结果是狼人。今天应该先出 P1。我是真预言家，信息绝对可靠。")
    # P1 Werewolf - LOW QUALITY: wolf perspective leak
    d1_sp1 = _make_speech(1, p1,
    ...
    d1_sp2 = _make_speech(1, p2,
        "我觉得 P1 一定是好人，P3 这个查杀太假了。P1 不可能是狼，谁投 P1 谁就有问题。"
        "我就保 P1 了，你们爱信不信。")
    # P4-P7 good speeches
    d1_sp4 = _make_speech(1, p4,
        "P3 的查杀信息比较关键，P1 发言确实不像好人视角。我倾向先出 P1。")
    d1_sp5 = _make_speech(1, p5,
        "先按查杀走，P1 的发言也不像好人视角，今天出 P1 没问题。")
    d1_sp6 = _make_speech(1, p6,
        "今天先出 P1，如果 P3 是假预言家，明天再处理。但 P1 刚才说漏嘴了，我投 P1。")
    d1_sp7 = _make_speech(1, p7,
        "我也觉得 P1 的解释很怪，说什么刀口……先投 P1。")
    ...
    events = [
        n1_tally, n1_wolf_kill1, n1_wolf_kill2,
        n1_witch_save, n1_seer_result, n1_seer_check, n1_guard,
        d1_sp3, d1_sp1, d1_sp2, d1_sp4, d1_sp5, d1_sp6, d1_sp7,
        d1_v1, d1_v2, d1_v3, d1_v4, d1_v5, d1_v6, d1_v7, d1_death,
        n2_tally, n2_wolf_kill,
        n2_witch_poison, n2_seer_result, n2_seer_check,
        n2_death_p7, n2_death_p2,
    ]
    ...
    decisions = [
        ...
        _make_decision(game_id, p1, Role.WEREWOLF, 1, "DAY_SPEECH", "TALK",
                       {"type": "speech", "speech": "你们别信 P3，他肯定是假预言家。我们狼……不是，我是说好人这边不可能这么快被查到。昨晚刀口也不是这么回事，P3 这个查杀太假了。"},
                       observation={"private": "我是狼人，P3 的查杀是真的"}),
        _make_decision(game_id, p2, Role.WEREWOLF, 1, "DAY_SPEECH", "TALK",
                       {"type": "speech", "speech": "我觉得 P1 一定是好人，P3 这个查杀太假了。P1 不可能是狼，谁投 P1 谁就有问题。我就保 P1 了。"},
        ...
        _make_decision(game_id, p3, Role.SEER, 1, "DAY_VOTE", "VOTE",
                       {"type": "vote", "target_id": "P1", "reasoning": "我查杀 P1"}),
        ...
        _make_decision(game_id, p3, Role.SEER, 2, "NIGHT_SEER", "DIVINE",
    ...
    return GameState(
        id=game_id, phase=Phase.GAME_END, day=3,
        players=all_players, events=events,
        decision_records=decisions, winner=Alignment.VILLAGE,
    )
...
def test_badcase_002_wolf_low_quality_scores_low(badcase_002_state: GameState) -> None:
    ...
    for pid in sorted(scores, key=lambda x: int(x[1:])):
        ps = scores[pid]
        print(f"  {pid} ({ps.role:10s}): process={ps.process_score:.2f} "
...
def test_badcase_002_detects_wolf_perspective_leak(badcase_002_opps) -> None:
    ...
    p1_speeches = [op for op in badcase_002_opps
                   if op.player_id == "P1" and op.opportunity_type == "speech"]
    ...
    assert leak > 0.3, \
        f"P1 speech should trigger wolf_perspective_leak, got {leak:.4f}"
...
def test_badcase_002_detects_teammate_overprotection(badcase_002_opps) -> None:
    ...
    p2_speeches = [op for op in badcase_002_opps
    ...
    if MODELS_EXIST:
        ...
        assert gap >= 0.05, \
            f"Overprotection should penalize q, gap={gap:.4f}"
        # Tightened: overprotection speech should be low
        assert cal.calibrated_q <= 0.55, \
            f"P2 overprotection speech should be low, got cal={cal.calibrated_q:.4f}"
...
def test_badcase_002_detects_vote_coordination_failure(badcase_002_opps) -> None:
    ...
    wolf_votes = [op for op in badcase_002_opps
                  if op.role == "Werewolf" and op.opportunity_type == "vote"]
...
def test_badcase_002_low_value_kill(badcase_002_opps) -> None:
    ...
    wolf_kills = [op for op in badcase_002_opps
    ...
    if n2_kills:
        ...
        if MODELS_EXIST:
            ...
            assert cal.calibrated_q <= 0.45, \
                f"N2 low-value kill should have cal_q <= 0.45, got {cal.calibrated_q}"
...
def test_badcase_002_dynamic_features_nonzero(badcase_002_opps) -> None:
    ...
    nonzero_counts = {f: 0 for f in wolf_dynamic_fields}
    ...
    assert active >= 4, \
        f"Expected >= 4 dynamic features with non-zero values, got {active}"
...
def test_badcase_002_uses_soft_calibration(badcase_002_opps) -> None:
    ...
    for op in badcase_002_opps:
        ...
        hard_caps = ["witch_poisoned_good", "hunter_shot_good",
                     "wolf_explicit_exposure_cap"]
    ...
    assert hard_cap_count == 0, \
        f"Hard caps detected ({hard_cap_count}), calibration should be 100% soft"
...
def test_badcase_002_process_score_v2_separates(badcase_002_state: GameState,
                                                  badcase_002_opps) -> None:
    ...
    assert p3_score > p1_score, \
        f"Seer ({p3_score:.4f}) should outscore leaking wolf P1 ({p1_score:.4f})"
    assert p3_score > p2_score, \
        f"Seer ({p3_score:.4f}) should outscore overprotective wolf P2 ({p2_score:.4f})"
