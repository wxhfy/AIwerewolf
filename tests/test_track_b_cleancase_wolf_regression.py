...
from backend.eval.scoring_models import (
    calibrate_decision_quality, compute_speech_scores,
    calculate_process_score_v2, extract_features, load_track_b_models,
)
...
def build_cleancase_001_fixture() -> GameState:
    ...
    ev = [_mwt(1, p3, {p1.id: "P3", p2.id: "P3"}),  # Kill Seer — high value
          _mna(1, p1, "attack", p3), _mna(1, p2, "attack", p3),
    ...
    ev += [_ms(1, p3, "我昨晚查验 P1，结果是狼人。今天应该先出 P1。我是真预言家。"),
           _ms(1, p1, "P3 跳得很急，但他没有解释为什么昨晚查我。大家可以看我昨天发言和投票，我没有帮任何人带节奏。如果P3是预言家，建议他明天再查一轮确认。"),
           _ms(1, p2, "P1这轮发言确实有压力，我不强保。P3如果是预言家，今天可以先按查杀走；但我会看明天刀口和发言再判断。大家先投票，我倾向跟票。"),
           _ms(1, p4, "跟查杀走，先出 P1。"),
    ...
    ev += [_mv(1, p1, p3), _mv(1, p2, p1),  # P2 votes teammate = smart sacrifice
           _mv(1, p3, p1), _mv(1, p4, p1), _mv(1, p5, p1), _mv(1, p6, p1), _mv(1, p7, p1),
           _md(1, p1, "vote")]

    # N2: P2 kills P4 Witch (high-value target)
    ev += [_mwt(2, p4, {p2.id: "P4"}), _mna(2, p2, "attack", p4),
    ...
    # Decisions
    decs = [
        _mdec(gid, p1, Role.WEREWOLF, 1, "NIGHT_WOLF", "KILL",
              {"type": "attack", "target_id": "P3", "reasoning": "刀预言家"}),
        _mdec(gid, p2, Role.WEREWOLF, 1, "NIGHT_WOLF", "KILL",
              {"type": "attack", "target_id": "P3", "reasoning": "刀预言家"}),
        _mdec(gid, p4, Role.WITCH, 1, "NIGHT_WITCH", "WITCH",
              {"type": "save", "target_id": "P3"}),
        _mdec(gid, p3, Role.SEER, 1, "NIGHT_SEER", "DIVINE",
              {"type": "divine", "target_id": "P1"}),
        _mdec(gid, p5, Role.GUARD, 1, "NIGHT_GUARD", "GUARD",
              {"type": "guard", "target_id": "P4"}),
        _mdec(gid, p3, Role.SEER, 1, "DAY_SPEECH", "TALK",
              {"type": "speech", "speech": "我昨晚查验 P1，结果是狼人。今天应该先出 P1。"}),
        _mdec(gid, p1, Role.WEREWOLF, 1, "DAY_SPEECH", "TALK",
              {"type": "speech", "speech": "P3跳得很急，但没有解释为什么昨晚查我。大家看我昨天发言和投票，我没有帮任何人带节奏。"},
              obs={"private": "我是狼人，P3的查杀正确"}),
        _mdec(gid, p2, Role.WEREWOLF, 1, "DAY_SPEECH", "TALK",
              {"type": "speech", "speech": "P1这轮发言确实有压力，我不强保。P3如果是预言家，今天可以先按查杀走；但我会看明天刀口和发言再判断。"},
              obs={"private": "P1是我的狼队友，我需要切割降低连坐风险"}),
        _mdec(gid, p4, Role.WITCH, 1, "DAY_SPEECH", "TALK",
              {"type": "speech", "speech": "跟查杀走，先出 P1。"}),
        _mdec(gid, p5, Role.GUARD, 1, "DAY_SPEECH", "TALK",
              {"type": "speech", "speech": "P3信息可信，先出 P1。"}),
        _mdec(gid, p6, Role.HUNTER, 1, "DAY_SPEECH", "TALK",
              {"type": "speech", "speech": "先出 P1 没问题。"}),
        _mdec(gid, p7, Role.VILLAGER, 1, "DAY_SPEECH", "TALK",
              {"type": "speech", "speech": "跟查杀投票。"}),
        _mdec(gid, p1, Role.WEREWOLF, 1, "DAY_VOTE", "VOTE",
              {"type": "vote", "target_id": "P3"}),
        _mdec(gid, p2, Role.WEREWOLF, 1, "DAY_VOTE", "VOTE",
              {"type": "vote", "target_id": "P1", "reasoning": "切割队友降低连坐"},
              obs={"private": "P1是我的狼队友，牺牲P1保自己"}),
        _mdec(gid, p3, Role.SEER, 1, "DAY_VOTE", "VOTE",
              {"type": "vote", "target_id": "P1"}),
        _mdec(gid, p4, Role.WITCH, 1, "DAY_VOTE", "VOTE",
              {"type": "vote", "target_id": "P1"}),
        _mdec(gid, p5, Role.GUARD, 1, "DAY_VOTE", "VOTE",
              {"type": "vote", "target_id": "P1"}),
        _mdec(gid, p6, Role.HUNTER, 1, "DAY_VOTE", "VOTE",
              {"type": "vote", "target_id": "P1"}),
        _mdec(gid, p7, Role.VILLAGER, 1, "DAY_VOTE", "VOTE",
              {"type": "vote", "target_id": "P1"}),
        _mdec(gid, p2, Role.WEREWOLF, 2, "NIGHT_WOLF", "KILL",
              {"type": "attack", "target_id": "P4", "reasoning": "刀女巫——高价值神职"},
              obs={"private": "P1已出局，我要刀女巫P4"}),
        _mdec(gid, p3, Role.SEER, 2, "NIGHT_SEER", "DIVINE",
              {"type": "divine", "target_id": "P2"}),
    ]

    p1.alive = False  # Voted out D1
    p4.alive = False  # Killed N2
    return GameState(id=gid, phase=Phase.GAME_END, day=3,
                     players=ap, events=ev, decision_records=decs,
...
def test_cleancase_001_good_wolf_speech_not_penalized(cleancase_001_opps) -> None:
    ...
    p2_speeches = [op for op in cleancase_001_opps
    ...
    print(f"\n  P2 speech leak={feats.wolf_perspective_leak_score:.4f} "
          f"overprotect={feats.teammate_overprotection:.4f}")
...
def test_cleancase_001_high_value_kill_scores_high(cleancase_001_opps) -> None:
    ...
    wolf_kills = [op for op in cleancase_001_opps
    ...
    for op in wolf_kills:
        ...
        if MODELS_EXIST:
            ...
            assert cal.calibrated_q >= 0.55, \
                f"High-value kill should score well, got {cal.calibrated_q}"
...
def test_cleancase_001_no_wolf_badcase_false_positive(cleancase_001_state) -> None:
    ...
    wolf_specific = [bc for bc in wolf_bad
                     if any(kw in bc.description
                            for kw in ["perspective", "leak", "overprotect", "硬保"])]
    assert len(wolf_specific) == 0, \
        f"Good wolf play should NOT trigger wolf-specific bad cases: {wolf_specific}"


def test_cleancase_001_process_score_high_for_good_wolf(cleancase_001_state,
                                                         cleancase_001_opps) -> None:
    ...
    assert p2_score >= 0.55, \
        f"Good wolf P2 should have decent score, got {p2_score:.4f}"
